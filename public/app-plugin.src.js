    ...
    init: function (options) {
        ...
        require.sync(initJsList, function ($, App, template, func, $messager, $utils, dialog) {
            ...
            require.sync([
                ...
                "bs-http-plugin/bs-camera",
                "bs-http-plugin/bs-pd",
                "bs-http-plugin/bs-pos",
                "bs-http-plugin/bs-print",
                "bs-http-plugin/bs-xsheet",
                "bs-http-plugin/data-transmit/socket"], function (Config,CA, DocCamera, PD, POS, Print,Xsheet, Socket) {
                window.bossJS = {
                    CA: CA,
                    DocCamera: DocCamera,
                    PD: PD,
        ...
        if(options.downloadUrl){
            window.bossJS.Config.downloadUrl = options.downloadUrl;
        }
        return bossJS;
    }
};
		...
		parseOptions: function(target, properties){
            ...
            if (o.objs&&o.objs.length>0&&id){
                ...
                if (initJsObj.uiExtConfig){
                    var uiExtConfig=initJsObj.uiExtConfig;
                    if (uiExtConfig[id]&&$.isFunction(uiExtConfig[id])){
                        options["beforeRender"]=function(config){
                            //重置初始化对象
                            var confighelper=new ConfigHelper(config);
                            uiExtConfig[id].call(this,confighelper);
                        };
...
define('bs-http-plugin/config-ext',["app/core/app-core", "app/widgets/window/app-messager"],function ($A, $messager) {
    var downloadPath = "bsPlugDownload/app?version=0";

    var config = {
        "bs-client-config":{
            //控件下载地址 //TODO 配置化从后端配置文件读取
            downloadUrl:$A.getHostUrl()+downloadPath,
...
define('bs-http-plugin/config',["app/core/app-jquery","app/core/app-core","bs-http-plugin/config-ext"], function (AppCore,$A,GlobalConfig) {
    ...
    var Config = {
        ...
        timeout:2000,
        sliceSize:1024,
        companyClient:'财政客户端综合管理组件'
    }
    if(location.href.match('^https://')!==null){
        Config.url = 'https://127.0.0.1:13525/';
        Config.guardUrl = 'https://127.0.0.1:13527/controlMainApp';
    }
...
define('bs-http-plugin/data-transmit/socket',["jquery",
    ...
    var urlMaxLen = 2000;
    var sendByIframeTimeout = 10*60*1000;
    //状态
    var STATE = {
        pending:{
            code:0,
...
define('app/util/app-utils',["app/core/app-jquery","app/core/app-core",
	...
	// 	 * chunkCount 拆分粒度
	// 	 * */
	// 	chunckArray :function(array, chunkCount){
	// 		var chunks = [],  i, j;
	...
	// };
		...
		"isMobile":function (mobileNum){
			return (new RegExp(/^\d{11}$/)).test(mobileNum);
		},
		/**
		 * 是否是邮件
		 */
...
define('bs-http-plugin/base-pluginvoke',["app/core/app-class",
    ...
    var BasePluginvoke = Class.create({
         ...
         *
         * @type {boolean}
         * @description 调用博思客户端助手失败时是否自动弹出帮助对话框,如果设置为true请配置url地址。
         * @memberOf base-pluginvoke
         * @example
        ...
        showHelp:function () {
            ...
            var html = '\
                    ...
                    <div style="box-sizing: border-box;height:100px;padding:40px 20px 20px 20px;;font-size:16px;font-weight:normal;text-align:center;">此功能需要安装    <span style="color:red;">'+Config.companyClient+'</span>   ，请前往下载中心自行下载。如已安装，请点击启动按钮</div> \
                    ...
                    <a id="startBtn" class="app-button l-btn l-btn-small" style="height:35px;background:#449d44;color:#fff;margin-right:20px;filter:none;">\
                            <span class="l-btn-left">\
                                <span class="l-btn-text" style="font-size:14px;padding-top:3px;">启动</span>\
                            </span>\
                        </a>\
                    <a id="handlerStartBtn" data-url="${startUrl}" class="app-button l-btn l-btn-small" \style="height:35px;background: rgba(206, 52, 38, 0.8);color:#fff;margin-right:10px;filter:none;display:none;">\
                            <span class="l-btn-left">\
                                <span class="l-btn-text" style="font-size:14px;padding-top:3px;">手动启动</span>\
                            </span>\
                        </a>\
                    <a id="downBtn" class="app-button l-btn l-btn-small" \style="height:35px;background:#ec971f;color:#fff;margin-right:10px;filter:none;">\
                            <span class="l-btn-left">\
                                <span class="l-btn-text" style="font-size:14px;padding-top:3px;">下载'+Config.companyClient+'</span>\
                            </span>\
                        </a>\
            ...
            $node.find('#downBtn').on('click',function () {
                // window.open($(this).data('url'),'下载'+Config.companyClient);
                var downloadUrl = Config.downloadUrl;
                var _platform = navigator.platform.toLowerCase();
                var isWindows = (_platform.indexOf("win") > -1);

                function createList(o) {
                    var str = "<ul class=\"selectUl\">";
                    for(var i=0, l = o.length; i<l; ++i) {
                        str += "<li oliValue='"+o[i].value+"' os='"+o[i].os+"' text='"+o[i].text+"'>"+o[i].text+"</li>";
                    }
                    str += "</ul>";
                    return str;
                }
                ...
                if(!isWindows){
                    $.ajax({
                        url: "platform/appframe/client/afaclientos/getBsClientOs.do",
                        type: "get",
                        dataType: "json",
                        async: false,
                        success: function(data) {
                            var systemUl = createList(data.systemList);
                            var architectureUl = createList(data.architectureList);

                            var txt=  "<div class=\"selectBox\">" +
                                "      <div class='xcLabel'>操作系统：</div>"+
                                "    \t<div class=\"inputCase displayInline\">\n" +
                                "\t\t\t\t<input id='systemOs' class=\"imitationSelect\" type=\"text\" text=\"\" oliValue=\"\" value=\"\"/>\n" +
                                "\t\t\t\t<i class=\"fa fa-caret-down\"></i>\n" +
                                "\t\t\t</div>\n" +
                                systemUl +
                                "<br/>"+
                                "      <div class='xcLabel' style='margin-top: 50px;'>CPU型号：</div>"+
                                "\t\t\t<div class=\"inputCase displayInline\" style='margin-top: 50px;'>\n" +
                                "\t\t\t\t<input id='architecture' class=\"imitationSelect\" type=\"text\" text=\"\" oliValue=\"\" value=\"\"/>\n" +
                                "\t\t\t\t<i class=\"fa fa-caret-down\"></i>\n" +
                                "\t\t\t</div>\n" +
                                architectureUl +
                                "        </div>";
                            __xcConfirm(txt,{onOk: function () {
                                    if(!$('#systemOs').val() || !$('#architecture').val()){
                                        alert('操作系统和CPU型号必须选择!');
                                        return;
                                    }
                                    window.location.href = downloadUrl + "&platform="+ $('#systemOs').attr('oliValue') + "&architecture="+ $('#architecture').attr('oliValue') + "&os=linux" ;
                                }});
                            ...
                            $(".selectUl li").click(function(event){
                                ...
                                var text = $(this).attr("text");//定义一个name属性，获取点击的元素属性赋值到当前，方便动态化传值
                                var oliValue = $(this).attr("oliValue");//定义一个id属性，获取点击的元素属性赋值到当前，方便数据交互传值
                                $(this).parent().prev().children().val(text); //把当前点击的name赋值到显示的input的val里面
                                $(this).parent().prev().children().attr("text",text);//把当前点击的oliName赋值到显示的input的oliName里面
                                $(this).parent().prev().children().attr("oliValue",oliValue);//把当前点击的oliId赋值到显示的input的oliId里面
                            });
            ...
            $node.find('#handlerStartBtn').on('click',function () {
                window.open($(this).data('url'),'通过浏览器手动启动'+Config.companyClient)
            });
            $node.find('#startBtn').on('click',function () {
                ...
                Socket.get({
                    ...
                    $A.messager.warn('启动失败，请尝试手动启动或重新安装'+Config.companyClient+'...');
                    $('#startBtn').hide();
        ...
        Statics:{
            ...
            heartbeat:function (time) {
                ...
                // var heartBeatIframe = document.getElementById("heartBeatIframe");
                // if(heartBeatIframe){
                //     document.body.removeChild(heartBeatIframe);
                // }
                // var elemIF = document.createElement("iframe");
                // elemIF.id='heartBeatIframe';
                // elemIF.src =  Config.url + Config.heartbeat + '?jsonp='+jsonp;
    ...
    __xcConfirm = function(popHtml, type, options) {
        var btnType = {
...
define('bs-http-plugin/bs-camera',['bs-http-plugin/base-pluginvoke',"bs-http-plugin/util/uuid"], function (BasePluginvoke,uuid) {

    /**
     * @desc 高拍仪 使用请引入 'bs-http-plugin/bs-camera'
     * @class
     * @classdesc
     * @name bs-camera
     * @extends base-pluginvoke
...
define('bs-http-plugin/bs-extend',['bs-http-plugin/base-pluginvoke'], function(BasePluginvoke,Socket) {
     ...
     * @extends base-pluginvoke
     */
    var Extend = BasePluginvoke.extend({
        /**
         * @description dll名称
         * @memberOf bs-extend
         * @instance
        ...
        functions:[
             ...
             * @memberOf bs-extend
             * @description 扩展调用
             * @param {object} op 业务参数
             * @param {string} op.dllName
             * @param {string} op.func
             * @param {object} op.payload 业务参数
             * @returns {object}
             *
             * @example
             * var instance = Extend.getInstance();
             * instance.invoke({
			 * 	dllName:'xxxx',
			 * 	func:'xxxx',
             *  payload:'xxxx'
...
define('bs-http-plugin/bs-print',['bs-http-plugin/base-pluginvoke',
    ...
    var BsPrint = BasePluginvoke.extend({
        ...
        functions: [
           ...
           /**
             * @function exportPagesTo
             * @instance
             * @memberOf bs-print
             * @description 批量导出的功能
             * @param {object} exportPath 导出文件的路径，导出文件需要包含后缀名。例如: 如我们需要将当前这些页面统一导出导 temp目录下,并将页面导出为xls文件，则路径设置为:“/temp/reports.xls“。
             * @param {object} exportFileName 导出名称
             * @param {object} fileSuffix: 'XLS' 
             * @param {object} sourcePages 需要导出的页面对象列表。客户端导出按照列表中对象的位置顺序导出
             * @returns {string}
             *
             * @example
             * var instance = getInstance();
             * instance.getPrintSet({ ModuleId: '00000000' }).then(function (PrintSet) {
             *     var dfd = $.Deferred();
             *     $.ajax({
             ...
             *     return dfd.promise();
             * }).then(function (data) {
             *     return instance.exportPagesTo({
             *         exportPath: 'C:/Users/admin/Desktop/111/',
             *         exportFileName: '3',
             *         fileSuffix: 'XLS',
             *         sourcePages: [
             *             {
             ...
             *                     }
             *                 },
             *                 LoadReportURL: $A.getHostUrl() + 'template/OnlyDetail' + ".grf"
             *             },
             *             {
             *                 Model: 'template',
             *                 PrintData:
             ...
             *                                 "CompanyName": "永大企业",
             *                                 "Address": "港务口街 29 号",
             *                                 "OrderDate": "1996-7-12 0:00:00",
             *                                 "Remark": "余小姐(089) 7034214"
             *                             }
             *                         ] 
             *                     }
             *                 },
             *                 LoadReportURL: $A.getHostUrl() + 'template/DetailParameters' + ".grf"
             *             },
             *             {
             *                 Model: 'D-Sheet',
             *                 PrintData: data.data
            ...
            */
            ...
            'getBarCodeInfo',
             /**
             * @function exportCustomPrintPages
             * @instance
             * @memberOf bs-print
             * @description  将自定义报表导出为指定格式的文件
             * @param {array} options - 包含多个option，其中option 是一个object
             *         
             *         option 
             *         {
             *              PrintData,  打印数据
             *              PrintSet,   打印设置
             *              ExportType, 导出文件类型,支持包括PDF,EXCEL,PNG
             *              ExportPath，指定导出路径，需要包含文件名。
             *          }
             * @returns {string}
             *
             * @example
             * var options = [];
             * 
             * /// 假设待导出5个页面, 这里将5个页面分别push到options中。
             * for(i=0; i<5; i++)
             * {
             *    options.push({
             *      "PrintData": { ... },
                    "PrintSet": { ... },
                    "ExportType": "PDF/Excel(默认PDF)",
                    "ExportPath": "/tmp/filename.pdf"    /// 临时文件路径
             *    })
             * }
             * 
             * 
             * var print = BsPrint.getInstance();
             * print.exportCustomPrintPages(options)
             * .done(function(data){
             * 		//TODO show image
             * }).fail(function(){
             * 		//TODO error
        ...
        exportCustomPrintPages: function (options) {
            var self = this;
            return self.openFile().then(function (path) {
              for (i = 0; i < options.length; i++) {
                options[i]['PrintPreview'] = false;
                options[i]['ExportPath'] = path + options[i]['ExportPath'];
              }
              return self.operate({
                data: options,
                func: 'exportCustomPrintPages',
              });
            });
        },

        exportPagesTo: function (options) { 
            options.extension = {version:'v1.00'};
        ...
        doPreview: function () {
            ...
            $.when(_self.initStateDefer)
                .done(function () {
                    ...
                    msgDlg&&$(document.body).css('overflow', 'hidden');
                    //打开对话框配置
                    var html = "";
                    var dlgOptions = {
...
define('bs-http-plugin/bs-qrcodepay',['bs-http-plugin/base-pluginvoke'], function(BasePluginvoke,Socket) {

    /**
     * @desc 二维码支付控件 使用请引入 'bs-http-plugin/bs-qrcodepay'
     * @class
     * @classdesc
     * @name bs-qrcodepay
     * @extends base-pluginvoke
     */
    var QrCodepay = BasePluginvoke.extend({
        /**
         * @description dll名称
         * @memberOf bs-qrcodepay
         * @instance
         * @example
         * module:'qrcodepay'
         */
        ...
        functions:[
             ...
             * @param {integer} options.data.device 设备类型 (0:联迪QM10)
             * @param {string} options.data.text 正文
             * @param {object} options.config 额外参数暂时未用到
             *
             * @returns {object}
             *
             * @example
             * var instance = QrCodepay.getInstance();
             * instance.QrcodePay({
			 * 	  device:0,
			 *    text:"xxxx"
			 * 	}).done(function(data){
        ...
        Statics:{
             ...
             * @param {string} op.config.appId 应用Id(<span style="color:red">必填</span>)
             * @example
             * 			//第二次获取就不需要options,即 QrCodepay.getInstance();
             *         var instantce = QrCodepay.getInstance({
             *          config:{
             *              appId:'123'
             *          }
             *         });
            ...
            getInstance:function(op){
                if (!this.instance){
                    this.instance =new QrCodepay(op);
                }
                return this.instance;
            }
        }
...
define('bs-http-plugin/bs-smart-card',['bs-http-plugin/base-pluginvoke'], function(BasePluginvoke,Socket) {
...
define('bs-http-plugin/bs-webbillcard',['bs-http-plugin/base-pluginvoke'], function(BasePluginvoke,Socket) {
...
define('bs-http-plugin/bs-xsheet',['bs-http-plugin/base-pluginvoke'], function(BasePluginvoke,Socket) {
     ...
     * @extends base-pluginvoke
     */
    var Xsheet = BasePluginvoke.extend({
        /**
         * @description dll名称
         * @memberOf bs-pos
         * @instance
        ...
        functions:[
 /**
             * @function printingSheetRequest
             * @instance
             * @memberOf bs-print
             * @description excel导出
             * @param {object} options 
             * @param {string} options.url ip+端口号
             * @param {string} options.fileType   导出类型，支持excel和pdf
             * @param {string} options.excelId   表格标识
             * @param {object} options.requestPayload    请求数据参数对象
             * @param {object} options.exportFilePath   导出路径
             *
             * @example
             ...
             *(
                {
                  url:'...',
                  fileType:'excel', //导出类型，支持excel和pdf
                  excelId:'...',
                  requestPayload:{...},
                  exportFilePath:''  ,
                }
...
(function (scope) {
    ...
    if(location.href.match('^https://')!==null){
        casUrl = 'https://127.0.0.1:13525/ca';
        heartUrl = 'https://127.0.0.1:13525/heart';
    }
    var Message = function () {
        var _self = this;
        _self.msgStack = {};
    ...
    CASLogin.prototype = {
        ...
        doSign:function (op) {
            ...
            var error = $.isFunction(op.error)?op.error:$.noop;
...
"bs-http-plugin/bs-qrcodepay",

"bs-http-plugin/bs-smart-card",

"bs-http-plugin/bs-webbillcard",
