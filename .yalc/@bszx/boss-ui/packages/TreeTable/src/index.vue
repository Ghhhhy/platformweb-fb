...
<script>
import BsTable from '../../Table/src/index.vue'
export default {
  name: 'BsTreeTable',
  components: { BsTable },
  props: {
  ...
  computed: {
    ...
    tableColumnsConfigMiddleWare() {
      const { isTreeSeqToFlat } = this.$attrs['tree-config'] || {}

      return [{
        title: '序号',
        type: isTreeSeqToFlat ? 'seq' : undefined,
        width: 60,
        field: '_index',
        fixed: 'left',
        cellRender: isTreeSeqToFlat ? {} : { name: 'seqRender' }
