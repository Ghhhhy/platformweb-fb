/*
 * @Author: 轻语
 * @Date: 2021-11-01 15:32:26
 * @LastEditors: xietingting
 * @LastEditTime: 2023-02-08 09:44:23
 * @Description:
 */
import BigNumber from './big.js'
import evalCalcUtil from './eval/index.js'

// 唯一标识
let uuid = 1
export default {
  eval: evalCalcUtil.eval,
  ...
  accurateFixedToRound(number, n) { // 保留小数位数(四舍五入)
    return (Number(number) || 0).toFixed(n)
  },
  ...
  accurateMaxFixed(number, n) { // 保留最多n位小数位数(正负数精确四舍五入)
    n = n ? parseInt(n) : 0
    if (n <= 0) {
      return Math.round(number)
    } else {
      return new BigNumber(number).round(n).toNumber()
    }
  },
  transToNumberLocalString(number, digits = 0, isRound) {
    // 返回逗号隔开千分位的数字
    let result = ''
  ...
  transToNumber(number, digits = 0, isRound) {
    ...
    if (isRound) {
      let result = ''
      // number = Number(number).toFixed(digits)
      number = (Math.round((Number(number) + Number.EPSILON) * 100) / 100).toFixed(digits)
      if (number.indexOf('-') === 0) {
        result = '-'
      }
      let numberArr = number.split('').filter((item, index) => {
      ...
      try {
        number = window.__BS_UI_GLOBAL_CONFIG__?.project?.name === 'dfr'
          ? this.accurateFixedToRound(number, digits)
          : this.accurateFixed(number, digits)
        if (isNaN(number)) {
          number = window.__BS_UI_GLOBAL_CONFIG__?.project?.name === 'dfr'
            ? this.accurateFixedToRound('', digits)
            : this.accurateFixed('', digits)
        }
        return number
      } catch (e) {
        return ''
  ...
  digitUppercase(money, type = 'money') {
    ...
    // digitUppercase(-111111111111111.1111) "负壹佰壹拾壹万壹仟壹佰壹拾壹亿壹仟壹佰壹拾壹万壹仟壹佰壹拾壹元壹角壹分"
    ...
    // 分离金额后用的数组，预定义
    let parts
    let chineseStr = money < 0 ? type === 'number' ? '负' : '负' : ''
  ...
  digitLowercase(str) { // 千分位转数字字符串
    return String(str).replace(/undefined|null|NaN|,/ig, '')
  },
