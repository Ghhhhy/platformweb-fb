...
export function defaultViewValueFormat(value, row, column, context) {
  ...
  if (render && render.name) {
    ...
    switch (render.name) {
      ...
      case '$vxeInput':
        let value = (val + '').replace(/null|NaN|undefined/ig, '')
        return (props.suffixIcon && props.suffixIcon === '%') ? (value || 0) + '%' : value
      case '$vxeSelect':
      ...
      case '$vxeEditDownTerm':
        const year = val.slice(0, 2) || '0'
        const month = val.slice(2, 4) || '0'
        const day = val.slice(4, 6) || '0'
        return `${year}年${month}月${day}天`
      default:
        // 业务组件类定义渲染器,导出取值处理
        const renderersMap = context.context.tableConfigCp.renderers || {}
        if (renderersMap[render.name] && row[column.field + '__viewSort']) {
          const text = (row[column.field + '__viewSort'] + '').replace(/null|undefined|root|NaN/ig, '')
          return text
        }
        return val
    }
...
export function getCellViewTitle(column, unit = '元') {
  ...
  switch (column.type) {
    case 'money':
      return column.title.replace(/\(万元\)/, '') + '(' + unit + ')'
    case 'seqIndex':
      return '序号'
    default:
      if (render && render.name) {
