...
<template>
  <vxe-modal
    ...
    resize
    transfer
    @close="$emit('update:exportModalVisible', false)"
  >
    <div class="export-module">
      ...
      >
        ...
        <vxe-form-item v-if="isShowHighOptions && exportModalFormData.isShowExportType" title="导出类型" :span="24">
          <template v-slot>
        ...
        <vxe-form-item v-if="isShowHighOptions && exportModalFormData.isShowExportHeadOptions" title="导出表头选项" :span="24">
          <template v-slot>
            <div class="vxe-checkbox-group fn-inline">
              <!-- <vxe-checkbox
        ...
        <vxe-form-item v-if="isShowHighOptions && exportModalFormData.isShowExportDatOptions" title="导出数据选项" field="exportOption" :span="24">
          <template v-slot>
        ...
        <vxe-form-item v-if="isShowHighOptions && exportModalFormData.isShowExportOtherOptions" title="其他导出选项" field="exportOption" :span="24">
          <template v-slot>
            <div class="vxe-checkbox-group">
              <vxe-checkbox
...
export default {
  ...
  data() {
    return {
      exportModalFormData: {
        ...
        isShowExportType: true, // 是否展示导出类型
        isShowExportHeadOptions: true, // 是否展示导出表头类型
        isShowExportDatOptions: true, // 是否展示导出数据类型
        isShowExportOtherOptions: true, // 是否展示其他类型
  ...
  computed: {
    isShowHighOptions() {
      return this.exportModalFormData.isShowHighOptions === undefined ? !(this.exportModalFormData.isExportOnlySourceField || this.exportModalFormData.isExportOnlyViewTitle) : this.exportModalFormData.isShowHighOptions
    }
  },
  methods: {
    deepCopy(obj) {
    ...
    setExportColumns(columns, ignoreColsTypes) {
      ...
      if (!columns.find(it => it.title === '序号')) columns.unshift({ title: '序号', field: 'seqIndex', type: 'seqIndex' })
    ...
    onExportClick() {
      ...
      exportModalFormData = this.valiAndSet(exportModalFormData)
      this.$emit('onExportClick', exportModalFormData, this)
    }
  },
  mounted() {},
  watch: {
