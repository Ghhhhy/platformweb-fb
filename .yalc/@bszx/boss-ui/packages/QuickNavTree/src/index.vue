...
<template>
  <div
    ref="quickNavAll"
    v-ClickOutside="setIsShowNavFalse"
    style="--width: -340px"
    :class="isFixedNav ? 'showFixedIco' : ''"
    class="quick-nav-box"
  >
    <div
      class="quick-nav-btn"
    ...
    >
      ...
      <em
        ...
        v-show="isShowNavIn"
        class="fn-inline"
        @click.stop="isFixedNav = !isFixedNav"
      ></em>
    </div>
    <div
      v-show="isFixedNav || isShowNavIn"
      id="quickNavAll"
      class="quick-nav"
      @click="setIsShowNav(false)"
    >
      <div class="quick-nav-main fn-inline" @click.stop>
        <div ref="refsWidth" class="quick-nav-menu fn-inline">
          <SideBar :nav-data="leftNavData" @onNavClick="onNavClick" />
          <div class="drag-width fn-inline" @mousedown="TextWidthChange"></div>
        </div>
        <div
          v-if="isRenderRightNav"
          id="quickNavRight"
          class="quick-nav-right fn-inline"
          @mousewheel="handlerMouserScroll"
        >
...
import SideBar from './treeSideBarMenu/SideBar.vue'
export default {
  name: 'BsQuickNavTree',
  components: {
    // leftMenu
  ...
  methods: {
    ...
    TextWidthChange(e) {
      ...
      document.onmousemove = (e) => {
        ...
        this.$refs.quickNavAll.style =
          '--width:' + -(dw + (e.clientX - dx)) + 'px'
        if (odivParent.offsetWidth <= 300) {
          // 当盒子缩小到一定范围内的时候，让他保持一个固定值，不再继续改变
          odivParent.style.width = '300px'
          this.$refs.quickNavAll.style = '--width:' + -300 + 'px'
        }
        ...
        if (odivParent.offsetWidth + odivParent.offsetLeft >= this.pdfWidth) {
          ...
          this.$refs.quickNavAll.style =
            '--width:' + -(this.pdfWidth - odivParent.offsetLeft) + 'px'
        }
        this.$emit(
          'fixedNavChange',
          this.isFixedNav,
          this.$refs.refsWidth.offsetWidth
        )
      }
...
.quick-nav-box {
  ...
  .quick-nav-btn:hover {
    ...
    // background: rgba($color: #5176EA, $alpha: .07);
  }
  .quick-nav-btn {
    cursor: pointer;
    transition: 0.3s;
  ...
  .quick-nav {
    ...
    .list-group {
      ...
      .list-group-item.active {
        ...
        span {
          color: var(--app-main-color-light);
        }
      }
...
.quickNavAll-enter-active,
.quickNavAll-leave-active {
  transition: all 0.3s;
  .quick-nav-main {
    transition: all 0.3s;
  }
