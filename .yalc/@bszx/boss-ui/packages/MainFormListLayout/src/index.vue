  ...
  <div class="T-mainFormListLayout-modulebox mainFormListLayout-modulebox">
    <div class="mmc">
      ...
      <div class="mmc-toolbar-search">
        ...
        <div ref="queryRef" class="mmc-search">
...
export default {
  ...
  methods: {
    ...
    onSplitPaneResize(leftWidth) {
      ...
      if (leftWidth > 1) {
        this.curSplitPaneLeftWidth = leftWidth
        this.leftVisible = true
        if (this.$lodash.keys(this.$store.state || {}).includes('leftContentDisplay')) {
          this.$store.commit('setLeftContentDisplay', true)
        }
      } else {
        this.leftVisible = false
        if (this.$lodash.keys(this.$store.state || {}).includes('leftContentDisplay')) {
          this.$store.commit('setLeftContentDisplay', false)
        }
      }
    }
  },
  ...
  watch: {
    '$store.state.showConditionStatus': {
      ...
      handler(val) {
        if (typeof(val) === 'boolean') {
          if (val) {
            this.$nextTick(() => {
              this.$refs.queryRef.style.display = 'block'
            })
          } else {
            this.$nextTick(() => {
              this.$refs.queryRef.style.display = 'none'
            })
          }
        }
      }
    },
    '$store.state.leftContentDisplay': {
      handler(val) {
    ...
    leftVisible(newValue) {
      ...
      if (this.$lodash.keys(this.$store.state || {}).includes('leftContentDisplay')) {
        this.$store.commit('setLeftContentDisplay', newValue)
      }
    },
