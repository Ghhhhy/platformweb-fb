...
export default {
  ...
  transToNumber(number, digits = 0, isRound) {
    //  任意字符过滤为数字
  ...
  getParamsMap(fn, args) { // 获取函数参数map
  ...
  replaceConditionAll(allExpression, data = {}, types = ['']) { // 替换表达式对象里的引用我哦真实数据 执行顺序，递归替换嵌套依赖部分-->转成字符串-->替换真实值
    return this.parseConditions(this.recursiveReplaceExpressionAll(this.deepCopy(allExpression, types)), data, types)
  },
  parseConditions(conditionsObj, data, types) {
    return JSON.parse(this.dearConditions(JSON.stringify(conditionsObj), data || {}, types))
  },
  dearConditions(format, data = {}, types = ['$userInfo', '$curNavModule', '$data', '']) { // 匹配$user() 等等等
    ...
    for (let i in types) {
      ...
      regUKey.map((key, indexkey) => {
        format = format.replace($key + '{' + key + '}', (typesValue[i][key] || data[key] || 0))
      })
    }
    // let regR = new RegExp('({[a-zA-Z0-9_]*})', 'ig')
    // let datasKey = format.match(regR) || []
    ...
    //     format = format.replace(reg, data[keyF] === null || data[keyF] === undefined || data[keyF] === '' ? '\'0\'' : ('\'' + data[keyF] + '\''))
    //   }
    // }
    return format
  },
  replaceExpressionAll(allExpression, data = {}, type = 'number', isRecursive = false) { // 替换表达式对象里的引用我哦真实数据 执行顺序，递归替换嵌套依赖部分-->转成字符串-->替换真实值
    if (isRecursive) {
      return this.replaceExpressionS(this.recursiveReplaceExpressionAll(this.deepCopy(allExpression)), data, type)
    } else {
      return this.replaceExpressionS(this.deepCopy(allExpression), data, type)
    }
    // replaceExpressionAll({a:'$a{b}*{aa}*${bb.0}*$d{e.c}',b:'{a}+$d{e.c}'},{aa:2,bb:[3],$a:{b:4},$d:{e:{c:5}}})
  },
  replaceExpressionS(expressionObj, data, type) { // 将表达式里面的大括号引用{}进行字符型替换格式化
    return JSON.parse(this.replaceExpression(JSON.stringify(expressionObj), data || {}, type))
  },
  recursiveReplaceExpressionAll(allExpression) { // 递归替换嵌套依赖表达式
    Object.keys(allExpression).forEach((key) => {
      let dowhile = true
      while (dowhile) {
        let regUKey = ((allExpression[key].match(/\$?[a-zA-Z0-9_.]{0,}\{[A-Za-z0-9_.]{1,}\}/ig) || []).filter((item) => {
          return item.startsWith('${') || item.startsWith('{')
        })).map((key, keyIndex) => {
          return key.replace(/\$?{|}/ig, '')
        })
        dowhile = !regUKey.every((keyc, indexkey) => {
          if (allExpression[keyc]) {
            allExpression[key] = allExpression[key].replace('{' + keyc + '}', '(' + allExpression[keyc] + ')')
            return false
          } else {
            return true
          }
    ...
    // recursiveReplaceExpression({
    //   b: '{a}',
    //   d: '{b}+{a}', // "({a})+{a}"
    //   e: '{b}+{d}+{a}' // "({a})+(({a})+{a})+{a}"
    // })
  },
  replaceExpression(format, data = {}, type = 'number') { // 匹配$azAZ0-9_${*}||${*}||{*}等等
    // type: 可选number || value,
    let self = this
    let $reference = Array.from(new Set(format.match(/\$?[a-zA-Z0-9_.]{0,}\{[A-Za-z0-9_.]{1,}\}/ig) || []))
    let types = Array.from(new Set($reference.map((item) => {
      return item.match(/\$?[a-zA-Z0-9_.]{0,}/ig)[0]
    }).concat(''))).sort().reverse()
    for (let i in types) {
      let $key = types[i]
      let regUKey = (format.match(new RegExp('(\\' + $key + '{[a-zA-Z0-9_.]{1,}})', 'ig')) || []).map((key, keyIndex) => {
      ...
      regUKey.map((key, indexkey) => {
        format = format.replace($key + '{' + key + '}', function (item, index, source) {
          if (/\$?[a-zA-Z0-9_.]{1,}/.test($key)) {
            return self.getReplacedExpression(key, data, type, $key)
          } else {
            return self.getReplacedExpression(key, data, type, false)
          }
        })
      })
    }
    return format
    // dearExpression('$a{b}*{aa}*${bb.0}*$d{e.c}', {aa:2,bb:[3],$a:{b:4},$d:{e:{c:5}}})
  },
  getReplacedExpression(expression, data, type, $key) {
    let resData = $key ? data && data[$key] : data
    if (resData) {
      let dataFieldArr = expression.split('.')
      let every = dataFieldArr.every((item, index) => {
        if (resData[item]) {
      ...
      if (every && resData) {
        return type === 'number' ? (isNaN(Number(resData)) ? 0 : Number(resData)) : resData
      } else {
        console.warn(`Not found ${expression} in`, data)
        return type === 'number' ? 0 : type === 'null' ? null : undefined
      }
    } else {
      console.warn(`Not found ${$key}{${expression}} in`, data)
      return type === 'number' ? 0 : type === 'null' ? null : undefined
    }
  }
}
