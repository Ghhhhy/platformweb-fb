    ...
    <div class="drawer-block">
      <ul>
        <li
          ...
          :key="log.logid"
        >
          <i v-show="index === 0" class="vxe-button--icon bs-log__circular base-font baseradioactive"></i>
          <div v-show="index" class="drawer-circular"></div>
          <div class="drawer-straight">{{ log.actionName }}</div>
          <div class="drawer-right">
            <div class="journal">
              <p>{{ log.nodeName }}</p>
              <p>操作人员：{{ log.actionUser }}&nbsp&nbsp&nbsp&nbsp{{ log.actionTime }}</p>
            </div>
            <div v-if="log.message !== ''" class="journal-buttom">
              <!--<p>{{ log.dutyName }}</p>-->
              <p>{{ log.message }}</p>
            </div>
          </div>
      ...
      <button v-if="isShow" class="Morebutton" @click="showMore">{{ !showMoreList ? '展开更多&nbsp' : '收起&nbsp' }} <img src="./imgs/drop-down-arrow.svg" alt=""></button>
  ...
  ul{
    ...
    li:first-child{
      ...
      .drawer-circular{
        ...
        width: 20px;
        height: 20px;
        background:url(./imgs/commit-icon.svg);
        border-radius: 50%;
      }
      .drawer-straight{
        position: relative;
