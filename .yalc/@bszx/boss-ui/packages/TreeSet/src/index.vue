...
<template>
  <div
    ...
    :style="{ '--tree-set-no__icon': treeConfigIn.radioGroup.length ? '0px' : '7px', '--title-padding': isHide ? '4px' : '4px' }"
  >
    <template v-if="treeConfigIn.title">
      <div v-if="!isHide" class="fn-inline aside-tree-show-leftvisible" @click="changeAside">
        <i class="ri-indent-decrease"></i>
      </div>
      <span class="tree-set__title">{{ treeConfigIn.title }}</span>
      <!-- <vxe-select v-if="treeConfigIn.select.options" v-model="treeConfigIn.select.cur" :placeholder="treeConfigIn.select.placeholder">
        <vxe-option v-for="item in options" :key="item.key" :value="item.label" :label="item.label" />
      </vxe-select> -->
    </template>
    <template v-else>
      <div v-if="!isHide" class="fn-inline aside-tree-show-leftvisible" @click="changeAside">
