...
var _default2 = {
  ...
  render: function render(h) {
    ...
    var hasUseTooltip = _vXETable.default._tooltip; // if (validOpts.autoPos) {
    //   setTimeout(() => {
    //     this.handleFocus(this.validFields || [])
    //   }, 100)
    // }
  ...
  methods: {
    getItems: function getItems() {
      var $children = this.findComponentDownward(this);
    ...
    triggerHeaderHelpEvent: function triggerHeaderHelpEvent(evnt, params, isContent) {
      ...
      var isCellOverflow = false;
      var content = '';
      var inputEl = overflowElem.querySelector('.vxe-input input');
      var textEl = overflowElem.querySelector('span.text');
      var textareaEl = overflowElem.querySelector('.vxe-textarea textarea');

      if (isContent && inputEl) {
        overflowElem = inputEl;
        content = inputEl.value;
        isCellOverflow = overflowElem.scrollWidth > overflowElem.clientWidth;
      } else if (isContent && textEl) {
        overflowElem = textEl;
        content = textEl.textContent;
        isCellOverflow = overflowElem.scrollWidth > overflowElem.clientWidth;
      } else if (isContent && textareaEl) {
        overflowElem = textareaEl;
        content = textareaEl.value; // .replace(/\r|\n/ig, '')

        isCellOverflow = overflowElem.scrollHeight > overflowElem.clientHeight;
      } else {
        content = (overflowElem.textContent || '').trim();
        isCellOverflow = overflowElem.scrollWidth > overflowElem.clientWidth;
      }
      ...
      if (content && isCellOverflow) {
        ...
        if ($tooltip) {
          $tooltip.open(overflowElem, content, 'formItem');
        }
      }
    },
    ...
    beginValidate: function beginValidate(type, callback) {
      ...
      if (data && formRules) {
        $children.forEach(function (_ref5) {
        ...
        return Promise.all(itemValids).then(function () {
          ...
          if (validOpts.autoPos) {
            _this5.validFields = validFields; // this.$nextTick(() => {
    ...
    findComponentUpward: function findComponentUpward(context, componentName) {
    ...
    findComponentDownward: function findComponentDownward(context) {
      var _this7 = this;

      var componentName = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "VxeFormItem";
      var isContinuousFindDownward = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
      var result = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : [];
      var children = context.$children;
      children.forEach(function (vm) {
        if (componentName === vm.$options.name) {
          result.push(vm);

          if (isContinuousFindDownward && Array.isArray(vm.$children) && vm.$children.length) {
            _this7.findComponentDownward(vm, componentName, isContinuousFindDownward, result);
          }
        } else {
          if (Array.isArray(vm.$children) && vm.$children.length) {
            _this7.findComponentDownward(vm, componentName, isContinuousFindDownward, result);
          }
        }
      });
      ...
      fields.some(function (property) {
        ...
        if (comp && comp.itemRender) {
          ...
          if (inputElem) {
            ...
            var inputEl = $el.querySelector('.vxe-input input');
            var textareaEl = $el.querySelector('.vxe-textarea textarea');
    ...
    updateStatus: function updateStatus(scope, itemValue) {
      ...
      if (property) {
        this.validItemRules('change', property, itemValue).then(function () {
          _this8.clearValidate(property);
        }).catch(function (_ref7) {
          var rule = _ref7.rule,
              rules = _ref7.rules;

          var rest = _ctor.default.find(_this8.invalids, function (rest) {
