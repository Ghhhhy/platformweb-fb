...
var Methods = {
  ...
  // 如果为虚拟树，将树结构拍平
  transExpandTreeTableData: function transExpandTreeTableData(data) {
    var result = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : [];
    var seq = arguments.length > 2 ? arguments[2] : undefined;
    var self = this;
    var treeExpandeds = this.treeExpandeds,
        treeOpts = this.treeOpts;
    data.forEach(function (row, index) {
      result.push(row);
      row.$$seq = seq ? seq + '.' + (index + 1) : index + 1;

      if (treeExpandeds.length && treeExpandeds.indexOf(row) >= 0 && Array.isArray(row[treeOpts.children])) {
        self.transExpandTreeTableData(row[treeOpts.children], result, row.$$seq);
      }
    });
    return result;
  },
  recursionfilterDataMethod: function recursionfilterDataMethod(data, filterMethod) {
    var _this2 = this;

    var leaf = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
    ...
    return data.map(function (item, index) {
      ...
      if (Array.isArray(item.children) && item.children.length) {
        var result = _this2.recursionfilterDataMethod(item.children, filterMethod, leaf, item);
        ...
        if (result.length) {
          ...
          if (leaf) {
            ...
            item.children = [];
            return filterMethod(item, index, parent);
          }
        }
      } else {
        return filterMethod(item, index, parent);
  ...
  handleTableData: function handleTableData(force) {
    ...
    var _ref = this.fuzzySearchConfig || {},
        fuzzySearchMethod = _ref.fuzzySearchMethod,
        _ref$enabled = _ref.enabled,
        enabled = _ref$enabled === void 0 ? false : _ref$enabled;

    var fullData = force ? this.updateAfterFullData() : this.afterFullData;

    if (enabled && typeof fuzzySearchMethod === 'function') {
      fullData = this.recursionfilterDataMethod(fullData, function (row, index, prow) {
        return fuzzySearchMethod({
          row: row,
          index: index,
          prow: prow,
    ...
    var fullList = fullData;

    if (treeConfig.scrollY && scrollYLoad) {
      fullList = this.transExpandTreeTableData(fullData);
    }

    this.afterFullRenderData = fullList;
    this.tableData = scrollYLoad ? fullList.slice(scrollYStore.startIndex, Math.max(scrollYStore.startIndex + scrollYStore.renderSize, 1)) : fullList.slice(0);
  ...
  loadTableData: function loadTableData(datas) {
    ...
    var keepSource = this.keepSource,
        ...
        sYOpts = this.sYOpts,
        scrollYStore = this.scrollYStore;
    var tableFullData = datas ? datas.slice(0) : []; // const scrollYLoad = (!treeConfig || treeConfig.scrollY) && sYOpts.enabled && sYOpts.gt > -1 && sYOpts.gt < tableFullData.length

    var scrollYLoad = (!treeConfig || treeConfig.scrollY) && sYOpts.enabled && sYOpts.gt > -1;
    ...
    return this.computeScrollLoad().then(function () {
      ...
      _this4.computeRowHeight();

      _this4.handleReserveStatus();

      _this4.checkSelectionStatus();

      return _this4.$nextTick().then(_this4.recalculate).then(_this4.refreshScroll);
    });
  },
  ...
  reloadColumn: function reloadColumn(columns) {
    ...
    return this.clearAll().then(function () {
      return _this7.loadColumn(columns);
    });
  },
  ...
  defineField: function defineField(record) {
    ...
    var rowkey = getRowkey(this);
    this.visibleColumn.forEach(function (_ref2) {
      var property = _ref2.property,
          editRender = _ref2.editRender;
  ...
  updateAfterFullData: function updateAfterFullData() {
    ...
    if (filterColumns.length) {
      tableData = tableData.filter(function (row) {
        return filterColumns.every(function (_ref3) {
          var column = _ref3.column,
              valueList = _ref3.valueList,
              itemList = _ref3.itemList;
  ...
  reloadCustoms: function reloadCustoms(customColumns) {
    ...
    return this.$nextTick().then(function () {
      _this16.mergeCustomColumn(customColumns);

      return _this16.refreshColumn().then(function () {
        return _this16.tableFullColumn;
      });
    });
  },
  ...
  restoreCustomStorage: function restoreCustomStorage() {
    ...
    if ((customConfig || $toolbar) && (isResizable || isVisible)) {
      ...
      _ctor.default.each(customMap, function (_ref4, field) {
  ...
  refreshScroll: function refreshScroll() {
    ...
    return this.clearScroll().then(function () {
      if (lastScrollLeft || lastScrollTop) {
        // 重置最后滚动状态
        _this18.lastScrollLeft = 0;
        _this18.lastScrollTop = 0; // 还原滚动状态
  ...
  handleGlobalKeydownEvent: function handleGlobalKeydownEvent(evnt) {
    ...
    if (this.isActivated) {
      this.preventEvent(evnt, 'event.keydown', null, function () {
        var filterStore = _this23.filterStore,
            isCtxMenu = _this23.isCtxMenu,
            ctxMenuStore = _this23.ctxMenuStore,
            editStore = _this23.editStore,
            editOpts = _this23.editOpts,
            editConfig = _this23.editConfig,
            mouseConfig = _this23.mouseConfig,
            mouseOpts = _this23.mouseOpts,
            keyboardConfig = _this23.keyboardConfig,
            keyboardOpts = _this23.keyboardOpts,
            treeConfig = _this23.treeConfig,
            treeOpts = _this23.treeOpts,
            highlightCurrentRow = _this23.highlightCurrentRow,
            currentRow = _this23.currentRow,
            bodyCtxMenu = _this23.bodyCtxMenu;
        ...
        if (operCtxMenu) {
          ...
          if (ctxMenuStore.showChild && hasChildrenList(ctxMenuStore.selected)) {
            _this23.moveCtxMenu(evnt, keyCode, ctxMenuStore, 'selectChild', 37, false, ctxMenuStore.selected.children);
          } else {
            _this23.moveCtxMenu(evnt, keyCode, ctxMenuStore, 'selected', 39, true, _this23.ctxMenuList);
          }
        } else if (keyboardConfig && mouseConfig && mouseOpts.area && _this23.handleKeyboardEvent) {
          ...
          if (selected.column.type === 'checkbox' || selected.column.type === 'selection') {
            _this23.handleToggleCheckRowEvent(evnt, selected.args);
          } else {
            _this23.triggerRadioRowEvent(evnt, selected.args);
          }
        } else if (isEsc) {
          // 如果按下了 Esc 键，关闭快捷菜单、筛选
          _this23.closeMenu();

          _this23.closeFilter(); // 如果是激活编辑状态，则取消编辑
          ...
          if (!isEditStatus) {
            ...
            if (selected.row && selected.column) {
              ...
              _this23.handleActived(selected.args, evnt);
            }
          }
        } else if (isContextMenu) {
          // 如果按下上下文键
          _this23._keyCtx = selected.row && selected.column && bodyCtxMenu.length;
          clearTimeout(_this23.keyCtxTimeout);
          _this23.keyCtxTimeout = setTimeout(function () {
            _this23._keyCtx = false;
          }, 1000);
            ...
            if (selected.row || actived.row) {
              ...
              if (hasShiftKey) {
                if (keyboardOpts.enterToTab) {
                  _this23.moveTabSelected(targetArgs, hasShiftKey, evnt);
                } else {
                  _this23.moveSelected(targetArgs, isLeftArrow, true, isRightArrow, false, evnt);
                }
              } else {
                if (keyboardOpts.enterToTab) {
                  _this23.moveTabSelected(targetArgs, hasShiftKey, evnt);
                } else {
                  _this23.moveSelected(targetArgs, isLeftArrow, false, isRightArrow, true, evnt);
                }
              }
              ...
              if (childrens && childrens.length) {
                ...
                _this23.setTreeExpand(currentRow, true).then(function () {
                  return _this23.scrollToRow(targetRow);
                }).then(function () {
                  return _this23.triggerCurrentRowEvent(evnt, params);
                });
              }
            }
          }
          ...
          if (isEditStatus) {
            // 如果按下了方向键
            if (selected.row && selected.column) {
              _this23.moveSelected(selected.args, isLeftArrow, isUpArrow, isRightArrow, isDwArrow, evnt);
            } else if ((isUpArrow || isDwArrow) && highlightCurrentRow) {
              // 当前行按键上下移动
              _this23.moveCurrentRow(isUpArrow, isDwArrow, evnt);
            }
          }
        } else if (isTab && keyboardConfig && keyboardOpts.isTab) {
          // 如果按下了 Tab 键切换
          if (selected.row || selected.column) {
            _this23.moveTabSelected(selected.args, hasShiftKey, evnt);
          } else if (actived.row || actived.column) {
            _this23.moveTabSelected(actived.args, hasShiftKey, evnt);
          }
        } else if (keyboardConfig && keyboardOpts.isDel && (isDel || (treeConfig && highlightCurrentRow && currentRow ? isBack && keyboardOpts.isArrow : isBack))) {
          if (isEditStatus) {
            ...
            if (selected.row || selected.column) {
              if (delMethod) {
                delMethod({
                  row: selected.row,
                  rowIndex: _this23.getRowIndex(selected.row),
                  column: selected.column,
                  columnIndex: _this23.getColumnIndex(selected.column),
                  $table: _this23
                });
              } else {
              ...
              if (isBack) {
                ...
                _this23.updateBodyTopRows();
              }
            } else if (isBack && keyboardOpts.isArrow && treeConfig && highlightCurrentRow && currentRow) {
              // 如果树形表格回退键关闭当前行返回父节点
              var _XEUtils$findTree = _ctor.default.findTree(_this23.afterFullData, function (item) {
              ...
              if (parentRow) {
                ...
                _this23.setTreeExpand(parentRow, false).then(function () {
                  return _this23.scrollToRow(parentRow);
                }).then(function () {
                  return _this23.triggerCurrentRowEvent(evnt, params);
                });
              }
  ...
  handleTargetLeaveEvent: function handleTargetLeaveEvent() {
    ...
    if (tooltipOpts.enterable) {
      this.tooltipTimeout = setTimeout(function () {
        var tooltip = _this25.$refs.tooltip;
  ...
  setCheckboxRow: function setCheckboxRow(rows, value) {
    ...
    rows.forEach(function (row) {
      return _this26.handleSelectRow({
        row: row
      }, !!value);
    });
    return this.$nextTick();
        ...
        if (value === -1) {
          ...
          _ctor.default.eachTree([row], function (item) {
            if (row === item || !checkMethod || checkMethod({
              ...
              _this27.handleCheckboxReserveRow(row, value);
            }
          }, treeOpts);
        } // 如果存在父节点，更新父节点状态
      ...
      if (reserve) {
        if (value) {
          selectRows.forEach(function (row) {
            checkboxReserveRowMap[getRowid(_this28, row)] = row;
          });
        } else {
  ...
  triggerCurrentRowEvent: function triggerCurrentRowEvent(evnt, params) {
    ...
    var preParams = this.currentParams;
    this.currentParams = params;
    this.setCurrentRow(params.row);

    if (isChange) {
      this.emitEvent('current-change', params, evnt, preParams);
    }
  },
  ...
  triggerCellClickEvent: function triggerCellClickEvent(evnt, params) {
    ...
    this.clickTimer = setTimeout(function () {
      var highlightCurrentRow = _this31.highlightCurrentRow,
          editStore = _this31.editStore,
          radioOpts = _this31.radioOpts,
          expandOpts = _this31.expandOpts,
          treeOpts = _this31.treeOpts,
          editConfig = _this31.editConfig,
          editOpts = _this31.editOpts,
          checkboxOpts = _this31.checkboxOpts,
      ...
      if (!triggerExpandNode && (expandOpts.trigger === 'row' || isExpandType && expandOpts.trigger === 'cell')) {
        _this31.triggerRowExpandEvent(evnt, params);
      } // 如果是树形表格


      if (treeOpts.trigger === 'row' || treeNode && treeOpts.trigger === 'cell') {
        _this31.triggerTreeExpandEvent(evnt, params);
      } // 如果点击了树节点
        ...
        if (!triggerExpandNode) {
          ...
          if (highlightCurrentRow) {
            if (!triggerCheckbox && !triggerRadio) {
              _this31.triggerCurrentRowEvent(evnt, params);
            }
          } // 如果是单选框


          if (!triggerRadio && (radioOpts.trigger === 'row' || isRadioType && radioOpts.trigger === 'cell')) {
            _this31.triggerRadioRowEvent(evnt, params);
          } // 如果是复选框


          if (!triggerCheckbox && (checkboxOpts.trigger === 'row' || isCheckboxType && checkboxOpts.trigger === 'cell')) {
            _this31.handleToggleCheckRowEvent(evnt, params);
          }
          ...
          if (editConfig) {
            if (editOpts.trigger === 'manual') {
              if (actived.args && actived.row === row && column !== actived.column) {
                _this31.handleChangeCell(evnt, params);
              }
      ...
      _this31.emitEvent('cell-click', params, evnt);
    }, 150);
  },
  ...
  handleAsyncRowExpand: function handleAsyncRowExpand(row) {
    ...
    return new Promise(function (resolve) {
      ...
      _this34.expandOpts.loadMethod({
        ...
        _ctor.default.remove(_this34.expandLazyLoadeds, function (item) {
          return item === row;
        });
  ...
  loadScrollYData: function loadScrollYData(evnt) {
    var afterFullData = this.afterFullData,
        ...
        scrollYLoad = this.scrollYLoad,
        treeOpts = this.treeOpts,
        afterFullRenderData = this.afterFullRenderData;
    ...
    if (isLoadData || scrollYStore.visibleIndex !== toVisibleIndex) {
      ...
      if (scrollYStore.visibleIndex > toVisibleIndex) {
        ...
        if (preload) {
          if (scrollYLoad && treeOpts.scrollY) {
            scrollYStore.startIndex = Math.max(0, Math.min(afterFullRenderData.length - renderSize, toVisibleIndex - marginSize));
          } else {
  ...
  computeScrollLoad: function computeScrollLoad() {
    ...
    return this.$nextTick().then(function () {
      var vSize = _this41.vSize,
          scrollXLoad = _this41.scrollXLoad,
          scrollYLoad = _this41.scrollYLoad,
          sYOpts = _this41.sYOpts,
          scrollYStore = _this41.scrollYStore,
          sXOpts = _this41.sXOpts,
          scrollXStore = _this41.scrollXStore,
          visibleColumn = _this41.visibleColumn,
          rowHeightMaps = _this41.rowHeightMaps;
      var tableBody = _this41.$refs.tableBody;
      var tableBodyElem = tableBody ? tableBody.$el : null;
      var tableHeader = _this41.$refs.tableHeader;
      ...
      if (tableBodyElem) {
        ...
        if (scrollYLoad) {
          ...
          if (!sYOpts.rSize) {
            ...
            scrollYStore.renderSize = sYOpts.rSize;
          }
  ...
  updateScrollYSpace: function updateScrollYSpace() {
    ...
    if (treeConfig.scrollY, scrollYLoad) {
      bodyHeight = afterFullRenderData.length * scrollYStore.rowHeight;
    }
  ...
  scrollToTreeRow: function scrollToTreeRow(row) {
      ...
      if (matchObj) {
        ...
        nodes.forEach(function (row, index) {
          if (index < nodes.length - 1 && !_this43.isTreeExpandByRow(row)) {
            rests.push(_this43.setTreeExpand(row, true));
          }
        });
      }
    }
    ...
    return Promise.all(rests).then(function () {
      return _tools.DomTools.rowToVisible(_this43, row);
    });
  },
   ...
   * 更新表头合计
   */
  updateBodyTopRows: function updateBodyTopRows() {
    var bodyTopRowsConfig = this.bodyTopRowsConfig,
        visibleColumn = this.visibleColumn;
    var enabled = bodyTopRowsConfig.enabled,
        dataMethod = bodyTopRowsConfig.dataMethod;

    if (enabled && _ctor.default.isFunction(dataMethod)) {
      this.bodyTopData = visibleColumn.length ? dataMethod({
