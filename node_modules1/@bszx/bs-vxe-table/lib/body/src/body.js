...
function renderColumn(h, _vm, $xetable, $seq, seq, rowid, fixedType, rowLevel, row, rowIndex, $rowIndex, _rowIndex, column, $columnIndex, columns, items, $$seq) {
  var _ref2;
  ...
  var _e = $xetable._e,
      ...
      rowId = $xetable.rowId,
      renderCellClass = $xetable.renderCellClass;
  ...
  var attrs = {
    ...
    rowId: row[rowId],
    colField: column.property || column.field
  };
...
function renderBodyTopRows(h, _vm, $xetable, $seq, rowLevel, fixedType, tableData, tableColumn) {
  try {
    var bodyTopRowsConfig = $xetable.bodyTopRowsConfig,
        _$xetable$bodyTopRows = $xetable.bodyTopRowsData,
        bodyTopRowsData = _$xetable$bodyTopRows === void 0 ? [] : _$xetable$bodyTopRows,
        _$xetable$bodyTopData = $xetable.bodyTopData,
        bodyTopData = _$xetable$bodyTopData === void 0 ? [] : _$xetable$bodyTopData,
        tooltipOpts = $xetable.tooltipOpts,
        overflowX = $xetable.overflowX,
        currentColumn = $xetable.currentColumn,
        columnKey = $xetable.columnKey,
    ...
    var enabled = bodyTopRowsConfig.enabled,
        rowStyle = bodyTopRowsConfig.rowStyle,
        rowClassName = bodyTopRowsConfig.rowClassName,
        spanMethod = bodyTopRowsConfig.spanMethod,
        cellStyle = bodyTopRowsConfig.cellStyle,
        bodyTopCellClassName = bodyTopRowsConfig.bodyTopCellClassName;

    if (enabled) {
      var _cellType = 'topRow';
      var renderType = bodyTopRowsData && bodyTopRowsData.length ? 'dataObj' : 'data';
      var renderData = (bodyTopRowsData && bodyTopRowsData.length ? bodyTopRowsData : bodyTopData) || [];
      return renderData.map(function (list, _rowIndex) {
        var $rowIndex = _rowIndex;
        return h('tr', {
          class: ['vxe-top--row vxe-body--row vxe-body--top-row', rowClassName ? _ctor.default.isFunction(rowClassName) ? rowClassName({
            $table: $xetable,
            _rowIndex: _rowIndex,
            $rowIndex: $rowIndex,
            fixed: fixedType,
          ...
          style: rowStyle ? _ctor.default.isFunction(rowStyle) ? rowStyle({
            $table: $xetable,
            _rowIndex: _rowIndex,
            $rowIndex: $rowIndex,
            fixed: fixedType,
          ...
          var align = column.align,
              bodyTopClassName = column.bodyTopClassName,
              showOverflow = column.showOverflow;
          var showAllTip = tooltipOpts.showAll || tooltipOpts.enabled;
          var isColGroup = column.children && column.children.length;
          var fixedHiddenColumn = fixedType ? column.fixed !== fixedType && !isColGroup : column.fixed && overflowX;
          var cellOverflow = _ctor.default.isUndefined(showOverflow) || _ctor.default.isNull(showOverflow) ? true : showOverflow;
          var showEllipsis = cellOverflow === 'ellipsis';
          var showTitle = cellOverflow === 'title';
          var showTooltip = cellOverflow === true || cellOverflow === 'tooltip';
          var topCellAlign = align || 'left';
          var hasEllipsis = showTitle || showTooltip || showEllipsis;
          var attrs = {
            'data-colid': column.id
          };
          ...
          tfOns.click = function (evnt) {
            $xetable.emitEvent('top-cell-click', Object.assign({
              cell: evnt.currentTarget
            }, params), evnt);
          };

          tfOns.dblclick = function (evnt) {
            $xetable.emitEvent('top-cell-dblclick', Object.assign({
              cell: evnt.currentTarget
            }, params), evnt);
          }; // 合并行或列
          ...
          var type = column.type === 'seq' || column.type === 'index' ? 'seq' : column.type;
          return h('td', {
            class: ['vxe-top--column vxe-body--column vxe-body--top-td', column.id, (_ref4 = {}, _defineProperty(_ref4, "col--".concat(topCellAlign), topCellAlign), _defineProperty(_ref4, "col--".concat(type), type), _defineProperty(_ref4, 'col--last', $columnIndex === tableColumn.length - 1), _defineProperty(_ref4, 'fixed--hidden', fixedHiddenColumn), _defineProperty(_ref4, 'col--ellipsis', hasEllipsis), _defineProperty(_ref4, 'col--current', currentColumn === column), _ref4), _tools.UtilTools.getClass(bodyTopClassName, params), _tools.UtilTools.getClass(bodyTopCellClassName, params)].concat(typeof renderCellClass === 'function' ? renderCellClass(params, _vm, $xetable) : renderCellClass),
            attrs: attrs,
            style: cellStyle ? _ctor.default.isFunction(cellStyle) ? cellStyle(params) : cellStyle : null,
            on: tfOns,
            key: columnKey ? column.id : $columnIndex
...
function renderRows(h, _vm, $xetable, $seq, rowLevel, fixedType, tableData, tableColumn) {
  ...
  tableData.forEach(function (row, $rowIndex) {
    ...
    rows.push(h('tr', {
      ...
      return renderColumn(h, _vm, $xetable, $seq, seq, rowid, fixedType, rowLevel, row, rowIndex, $rowIndex, _rowIndex, column, $columnIndex, tableColumn, tableData, treeConfig.scrollY && scrollYLoad ? row.$$seq : undefined);
    }))); // 如果行被展开了
    ...
    if (!(treeConfig.scrollY && scrollYLoad) && treeConfig && treeExpandeds.length) {
...
var _default = {
  ...
  render: function render(h) {
    ...
    h('tbody', {
      ref: 'tbody'
    }, renderBodyTopRows(h, this, $xetable, '', 0, fixedType, tableData, tableColumn).concat(renderRows(h, this, $xetable, '', 0, fixedType, tableData, tableColumn)) // renderRows(h, this, $xetable, '', 0, fixedType, tableData, tableColumn)
    )]),
    /**
     * 选中边框线
     */
