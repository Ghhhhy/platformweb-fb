...
function getFooterContent(h, params) {
  ...
  if (renderType === 'dataObj') {
    return _columnIndex === 0 && ['seq', 'checkbox', 'radio'].indexOf(column.type) > 0 ? items._FrstColLable || '' : [_tools.UtilTools.formatText(items[column.property] || '', 1)];
  } else {
...
var Cell = {
  ...
  renderIndexCell: function renderIndexCell(h, params) {
    ...
    return [_tools.UtilTools.formatText(seqMethod ? seqMethod(params) : level ? $$seq || "".concat($seq, ".").concat(seq) : (seqOpts.startIndex || startIndex) + seq, 1)];
  },
