...
function renderColumn(h, _vm, $xetable, $seq, seq, rowid, fixedType, rowLevel, row, rowIndex, $rowIndex, _rowIndex, column, $columnIndex, columns, items, $$seq) {
  ...
  const attrs = { 'data-colid': column.id, rowId: row[rowId], colField: column.property || column.field }
  ...
  const params = { $table: $xetable, $seq, $$seq, seq, rowid, row, rowIndex, $rowIndex, _rowIndex, column, columnIndex, $columnIndex, _columnIndex, fixed: fixedType, type: cellType, isHidden: fixedHiddenColumn, level: rowLevel, visibleData: afterFullData, data: tableData, items }
  // 虚拟滚动不支持动态高度
  ...
  return h('td', {
    ...
    }, UtilTools.getClass(className, params), UtilTools.getClass(cellClassName, params)].concat(typeof (renderCellClass) === 'function' ? renderCellClass(params, _vm, $xetable) : renderCellClass),
...
function renderBodyTopRows(h, _vm, $xetable, $seq, rowLevel, fixedType, tableData, tableColumn) {
  try {
    const { bodyTopRowsConfig, bodyTopRowsData = [], bodyTopData = [], tooltipOpts, overflowX, currentColumn, columnKey, renderCellClass } = $xetable
    let { enabled, rowStyle, rowClassName, spanMethod, cellStyle, bodyTopCellClassName } = bodyTopRowsConfig
    if (enabled) {
      let cellType = 'topRow'
      let renderType = bodyTopRowsData && bodyTopRowsData.length ? 'dataObj' : 'data'
      let renderData = (bodyTopRowsData && bodyTopRowsData.length ? bodyTopRowsData : bodyTopData) || []
      return renderData.map((list, _rowIndex) => {
        const $rowIndex = _rowIndex
        return h('tr', {
          class: ['vxe-top--row vxe-body--row vxe-body--top-row', rowClassName ? XEUtils.isFunction(rowClassName) ? rowClassName({ $table: $xetable, _rowIndex, $rowIndex, fixed: fixedType, type: cellType }) : rowClassName : ''],
          style: rowStyle ? (XEUtils.isFunction(rowStyle) ? rowStyle({ $table: $xetable, _rowIndex, $rowIndex, fixed: fixedType, type: cellType }) : rowStyle) : null
        }, tableColumn.map((column, $columnIndex) => {
          const { align, bodyTopClassName, showOverflow } = column
          const showAllTip = tooltipOpts.showAll || tooltipOpts.enabled
          const isColGroup = column.children && column.children.length
          const fixedHiddenColumn = fixedType ? column.fixed !== fixedType && !isColGroup : column.fixed && overflowX
          const cellOverflow = (XEUtils.isUndefined(showOverflow) || XEUtils.isNull(showOverflow)) ? true : showOverflow
          let showEllipsis = cellOverflow === 'ellipsis'
          const showTitle = cellOverflow === 'title'
          const showTooltip = cellOverflow === true || cellOverflow === 'tooltip'
          const topCellAlign = align || 'left'
          let hasEllipsis = showTitle || showTooltip || showEllipsis
          ...
          const params = { $table: $xetable, _rowIndex, $rowIndex, column, columnIndex, $columnIndex, _columnIndex, itemIndex, items: list, fixed: fixedType, type: cellType, data: bodyTopRowsData, renderType }
          // 虚拟滚动不支持动态高度
          ...
          tfOns.click = evnt => {
            $xetable.emitEvent('top-cell-click', Object.assign({ cell: evnt.currentTarget }, params), evnt)
          }
          tfOns.dblclick = evnt => {
            $xetable.emitEvent('top-cell-dblclick', Object.assign({ cell: evnt.currentTarget }, params), evnt)
          }
          // 合并行或列
          if (spanMethod) {
            // 自定义合并函数
          ...
          const type = column.type === 'seq' || column.type === 'index' ? 'seq' : column.type
          return h('td', {
            class: ['vxe-top--column vxe-body--column vxe-body--top-td', column.id, {
              [`col--${topCellAlign}`]: topCellAlign,
              [`col--${type}`]: type,
              'col--last': $columnIndex === tableColumn.length - 1,
              'fixed--hidden': fixedHiddenColumn,
              'col--ellipsis': hasEllipsis,
              'col--current': currentColumn === column
            }, UtilTools.getClass(bodyTopClassName, params), UtilTools.getClass(bodyTopCellClassName, params)].concat(typeof (renderCellClass) === 'function' ? renderCellClass(params, _vm, $xetable) : renderCellClass),
            attrs,
            style: cellStyle ? (XEUtils.isFunction(cellStyle) ? cellStyle(params) : cellStyle) : null,
            on: tfOns,
            key: columnKey ? column.id : $columnIndex
  ...
  render(h) {
    ...
    return h('div', {
      ...
      h('table', {
        ...
        renderBodyTopRows(h, this, $xetable, '', 0, fixedType, tableData, tableColumn).concat(renderRows(h, this, $xetable, '', 0, fixedType, tableData, tableColumn))
