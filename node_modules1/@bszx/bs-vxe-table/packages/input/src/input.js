...
export default {
  ...
  methods: {
    ...
    triggerEvent(evnt) {
      ...
      // this.$emit(evnt.type, { $panel: $refs.panel, value: isDatePicker ? value : inputValue, $event: evnt }, evnt)
      this.$emit(evnt.type, { $input: this, $panel: $refs.panel, value: inputValue, $event: evnt }, evnt, this)
    },
    emitUpdate(value, evnt) {
      ...
      if (XEUtils.toString(this.value) !== value) {
        this.$nextTick(() => {
          this.$emit('change', { value, $event: evnt, $input: this })
        })
      }
    },
    inputEvent(evnt) {
     ...
     */
    initValue() {
      const { type, value, digitsValue } = this
      this.changeValue()
      if (type === 'float') {
        if (value) {
