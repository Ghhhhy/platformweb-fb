...
function renderCustoms(h, _vm) {
  ...
  XEUtils.eachTree(columns, (column) => {
    let colTitle = UtilTools.formatText(column.getTitle(), 1)
    colTitle = colTitle.replace(/\s+/ig, '') ? colTitle : (column.type === 'checkbox' ? '选择' : column.type === 'radio' ? '单选' : colTitle)
...
export default {
  ...
  methods: {
    ...
    closeCustom(isperform) {
      const { custom, setting, customStore } = this
      if (customStore.visible) {
        customStore.visible = false
        if ((custom || setting) && !customStore.immediate && isperform) {
          this.handleCustoms()
        }
      }
    },
