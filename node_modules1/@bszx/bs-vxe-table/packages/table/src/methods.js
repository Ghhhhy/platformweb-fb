...
const Methods = {
  ...
  // 如果为虚拟树，将树结构拍平
  transExpandTreeTableData(data, result = [], seq) {
    const self = this
    const { treeExpandeds, treeOpts } = this
  ...
  recursionfilterDataMethod(data, filterMethod, leaf = true, parent) { // 过滤树形数据
    return data.map((item, index) => { return Object.assign({}, item) }).filter((item, index) => {
      if (Array.isArray(item.children) && item.children.length) {
        let result = this.recursionfilterDataMethod(item.children, filterMethod, leaf, item)
        if (result.length) {
  ...
  handleTableData(force) {
    const { scrollYLoad, scrollYStore, treeConfig } = this
    const { fuzzySearchMethod, enabled = false } = this.fuzzySearchConfig || {}
    let fullData = force ? this.updateAfterFullData() : this.afterFullData
    if (enabled && typeof (fuzzySearchMethod) === 'function') {
      fullData = this.recursionfilterDataMethod(fullData, (row, index, prow) => {
        return fuzzySearchMethod({
          row, index, prow, columns: this.columns
        }, this)
      }, !!treeConfig)
    }
  ...
  loadTableData(datas) {
    ...
    // const scrollYLoad = (!treeConfig || treeConfig.scrollY) && sYOpts.enabled && sYOpts.gt > -1 && sYOpts.gt < tableFullData.length
    const scrollYLoad = (!treeConfig || treeConfig.scrollY) && sYOpts.enabled && sYOpts.gt > -1
  ...
  triggerCurrentRowEvent(evnt, params) {
    ...
    let preParams = this.currentParams
  ...
  loadScrollYData(evnt) {
    const { afterFullData, scrollYStore, isLoadData, scrollYLoad, treeOpts, afterFullRenderData } = this
  ...
  // 更新纵向 Y 可视渲染上下剩余空间大小
  updateScrollYSpace() {
    const { elemStore, scrollYStore, scrollYLoad, afterFullData, treeConfig, afterFullRenderData } = this
   ...
   */
  updateBodyTopRows() {
    let { bodyTopRowsConfig, visibleColumn } = this
    let { enabled, dataMethod } = bodyTopRowsConfig
    if (enabled && XEUtils.isFunction(dataMethod)) {
      this.bodyTopData = visibleColumn.length ? dataMethod({ columns: visibleColumn, data: this.afterFullData, $table: this, $grid: this.$xegrid }) : []
    }
    return this.$nextTick()
  },
  /**
