...
function getFooterContent (h, params) {
  const { $table, column, _columnIndex, items, renderType } = params
  ...
  if (renderType === 'dataObj') {
    return _columnIndex===0 && ['seq', 'checkbox', 'radio'].indexOf(column.type)>0 ? (items._FrstColLable || '') : [UtilTools.formatText(items[column.property] || '', 1)]
  } else {
...
export const Cell = {
  ...
  renderIndexCell (h, params) {
    ...
    const { $seq, $$seq, seq, level } = params
    // 在 v3.0 中废弃 startIndex、indexMethod
    const seqMethod = seqOpts.seqMethod || column.seqMethod || column.indexMethod
    return [UtilTools.formatText(seqMethod ? seqMethod(params) : level ? ($$seq || `${$seq}.${seq}`) : (seqOpts.startIndex || startIndex) + seq, 1)]
  },
