...
export default {
  ...
  render(h) {
    const { _e, $slots, loading, vSize, tooltipOpts, validOpts } = this
  ...
  methods: {
    getItems() {
      const $children = this.findComponentDownward(this)
      return $children.map(({ field, title, itemRender }) => {
        return {
          field,
          title,
          itemRender
    ...
    reset() {
      ...
      if (data) {
        const $children = this.findComponentDownward(this)
        $children.forEach(({ field, resetValue, itemRender }) => {
          if (field) {
    ...
    triggerHeaderHelpEvent(evnt, params, isContent) {
      const { item, showTooltip } = params
      if (showTooltip === false) {
        return
      }
      const { tooltipStore } = this
      const $tooltip = this.$refs.tooltip
      let overflowElem = evnt.currentTarget
      let isCellOverflow = false
      let content = ''
      const inputEl = overflowElem.querySelector('.vxe-input input')
      const textEl = overflowElem.querySelector('span.text')
      const textareaEl = overflowElem.querySelector('.vxe-textarea textarea')
    ...
    beginValidate(type, callback) {
      ...
      this.clearValidate()
      if (data && formRules) {
        $children.forEach(({ field, visible }) => {
    ...
    findComponentDownward(context, componentName = "VxeFormItem", isContinuousFindDownward = true, result = []) {
      let children = context.$children;
      children.forEach((vm) => {
        if (componentName === vm.$options.name) {
          result.push(vm)
          if (isContinuousFindDownward && Array.isArray(vm.$children) && vm.$children.length) {
            this.findComponentDownward(vm, componentName, isContinuousFindDownward, result)
          }
        } else {
      ...
      fields.some(property => {
        ...
        if (comp && comp.itemRender) {
          ...
          if (inputElem) {
            ...
            const inputEl = $el.querySelector('.vxe-input input')
            const textareaEl = $el.querySelector('.vxe-textarea textarea')
            if (inputEl) {
              inputElem.click()
              inputEl.focus()
            } else if (textareaEl) {
