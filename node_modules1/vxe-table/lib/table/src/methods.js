    ...
    if (this.isActivated) {
      this.preventEvent(evnt, 'event.keydown', null, function () {
        ...
        if (operCtxMenu) {
            ...
            if (selected.row || actived.row) {
              ...
              if (hasShiftKey) {
                if (keyboardConfig.enterToTab) {
                  _this19.moveTabSelected(targetArgs, hasShiftKey, evnt);
                } else {
                  _this19.moveSelected(targetArgs, isLeftArrow, true, isRightArrow, false, evnt);
                }
              } else {
                if (keyboardConfig.enterToTab) {
                  _this19.moveTabSelected(targetArgs, hasShiftKey, evnt);
                } else {
                  _this19.moveSelected(targetArgs, isLeftArrow, false, isRightArrow, true, evnt);
                }
              }
          ...
          // 如果按下了 Tab 键切换
          if (selected.row || selected.column) {
            _this19.moveTabSelected(selected.args, hasShiftKey, evnt);
          } else if (actived.row || actived.column) {
            _this19.moveTabSelected(actived.args, hasShiftKey, evnt);
          }
  ...
  handleAsyncTreeExpandChilds: function handleAsyncTreeExpandChilds(row) {
    ...
    return new Promise(function (resolve) {
      ...
      loadMethod({
        ...
        if (childRecords) {
          _this33.loadChildren(row, childRecords).then(function (childRows) {
            ...
            if (!checkStrictly && _this33.isCheckedByCheckboxRow(row)) {
              _this33.setCheckboxRow(childRows, true);
            }
          });
        }
