<template>
  <div id="app" :class="classMap" :style="{ height: wrapHeight }">
    <div class="app-main">
      ...
      <router-view v-if="!isMicroApp && ifrouteractive " />
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  computed: {
    isMicroApp() {
      return window.mircoApp
    }
  },
  methods: {
    /**
     * 查找点的菜单有没有配置路由页面
     * @param name {String}  页面路由名
     * @param routeList {Array} 路由合集
     * @returns 是否匹配到
     */
    hasRoute: function hasRoute(name, routeList) {
      !routeList && (routeList = this.$router.options.routes)
      // iframe嵌入url也通过匹配
      if (name.includes('.html')) return true
      for (var i = 0; i < routeList.length; i++) {
        if (routeList[i].name === name) {
          return true
        }
        if (routeList[i].children) {
          var flag = this.hasRoute(name, routeList[i].children)
          if (flag) {
            return flag
          }
        }
    ...
    /**
     * @description: 基座传递的数据包data发生变化
     * @Author: Comer
     * @Date: 2022-04-15 11:36:51
     * @param {*} data
     * @return {*}
     */
    baseDataListener(data) {
      if (data.type === 'refresh') {
        this.$refs.keepAlive && this.$refs.keepAlive.destroy(data.name)
      ...
      console.log('data557788', this.initMicro, data.name, this.$route.name, data)
      this.$store.commit('setCurNavModule', data.curNavModule)
      if (this.initMicro) {
        this.initMicro = false
        return
      }
      if (!this.hasRoute(data.name)) {
        setTimeout(() => {
          window.microApp.dispatch({ type: '无菜单信息' })
        }, 100)
      } else {
          ...
          this.$router.push(
            {
              name: data.name,
              params: {
                curNavModule: data.curNavModule
              }
            }
          )
        }
    ...
    /**
     * @description: 通过以下方法可以向基项目发送数据
     * @Author: Comer
     * @Date: 2022-04-15 11:37:13
     * @param {*} name
     * @return {*}
     */
    emitSub (name) {
    ...
    async getMenuInfo(tokenid) {
      await MenuModule.getMenuInfo().then((res) => {
        if (Array.isArray(res)) {
          const data = window.microApp.getData()
          this.ifrouteractive = true
          this.$store.commit('setUserInfo', data.menuInfo)
          window.localStorage.setItem('user', JSON.stringify(data.menuInfo))
          this.$store.commit('setCurNavModule', data.curNavModule)
          this.$router.push(
            {
              name: data.name,
  ...
  async created () {
    ...
    if (window.microApp) {
      window.microApp && window.microApp.addDataListener(this.baseDataListener, true)
      // 使用 window.microApp.getData() 方法获取微前端应用中的数据，将其存入 data 常量中
      const data = window.microApp.getData()
      // 使用 Vuex 的 commit() 方法将 data 中的 curNavModule 属性值存入状态管理器中，解决首次进入getPageConfig接口参数问题
      this.$store.commit('setCurNavModule', data.curNavModule)
      this.$store.commit('setCurMenuObj', data.curNavModule)
      this.getMenuInfo()
    }
  },
  destroyed() {
    window.microApp && window.microApp.removeDataListener(this.baseDataListener)
  },
