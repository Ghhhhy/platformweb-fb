...
<script>
import { proconf } from './InquiryLetterReplyByAgency'
...
export default {
  ...
  methods: {
    ...
    queryTableDatasCount() {
      const params = {
        statusCodeArr: ['2', '3', '2,3'],
        menuId: this.menuId
      }
      HttpModule.queryTableDatasCount(params).then(res => {
        if (res.code === '000000') {
          this.tabStatusNumConfig['2'] = res.data.unHandle
          this.tabStatusNumConfig['3'] = res.data.handle
          this.tabStatusNumConfig['2,3'] = res.data.allCount
        }
      })
    },
    // 查询 table 数据
