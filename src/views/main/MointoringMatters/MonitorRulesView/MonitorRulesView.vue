...
export default {
  ...
  computed: {
    ...
    isPermission() {
      ...
      const roleSet = new Set([...(this.userRolesData.map(item => item.name) || []), ...dfrRoles])
  ...
  methods: {
    ...
    operationToolbarButtonClickEvent(obj, context, e) {
      ...
      switch (obj.code) {
        ...
        case 'open':
          ...
          datas.forEach(item => {
            if (Number(item.regulationType) === 1) {
              isSystem = true
            }
          })
          ...
          if (datas.length === 1 && datas[0].isEnable === '1') {
            this.$message.warning('此规则已启用，无需再次启用！')
            return
          } else {
            let isenables = false
            datas.forEach(item => {
            ...
            if (isenables) {
              this.$message.warning('部分规则已启用，请重新选择！')
              return
            }
          }
          this.open(datas)
    ...
    queryTableDatas() {
      let param = {
        ...
        isDir: this.isDir,
        jurisdiction: (this.param5 && this.param5.jurisdiction === '1') || this.$route.name === 'SXMonitorRulesView',
      ...
      if (!this.isSx) {
        ...
        if ((this.$route.name === 'MonitorRulesView' || this.$route.name === 'MonitorRulesViewShow') && this.treeType === '2') {
    ...
    getLeftTreeData1() {
      if (this.showMonitorTree && this.isZDZJ) { // 如果是直达资金监控规则库 显示预警级别树
        this.treeData = proconf.leftYjjbData
        return
      }
      let param = {
        regulationClass: this.regulationClass
      ...
      HttpModule.getLeftTree1(param).then(res => {
        if (res.code === '000000') {
          let arr = []
          arr.push(res.data)
          let treeResdata = this.getChildrenData(arr)
  ...
  created() {
    this.param5 = this.transJson(this.$store.state.curNavModule.param5) // 如果是直达资金监控规则库 菜单参数配置dfr
