...
<script>
import { proconf } from './MonitoringRuleFunctionManagement'
...
export default {
  ...
  methods: {
    ...
    search(val) {
      ...
      if (this.searchDataList.functionType && this.searchDataList.functionType.trim() !== '') {
        condition.functionType = this.searchDataList.functionType
      }
      if (this.searchDataList.functionName && this.searchDataList.functionName.trim() !== '') {
        condition.functionName = this.searchDataList.functionName
      }
      if (this.searchDataList.functionParameter && this.searchDataList.functionParameter.trim() !== '') {
        condition.functionParameter = this.searchDataList.functionParameter
      }
    ...
    delPolicies() {
      ...
      let param = {
        functionCode: selection[0].functionCode
      }
    ...
    queryTableDatas() {
      const param = {
        ...
        functionName: this.condition.functionName ? this.condition.functionName.toString() : '',
        functionType: this.condition.functionType ? this.condition.functionType.toString() : '',
        businessModuleName: this.condition.businessModuleName ? this.condition.businessModuleName.toString() : '',
        functionParameter: this.condition.functionParameter ? this.condition.functionParameter.toString() : '',
        functionApi: this.condition.functionApi ? this.condition.functionApi.toString() : ''
      }
      this.tableLoading = true
    ...
    async getbusLists() {
      const { data } = await api.getbusLists({
        businessType: 2,
        parentId: '1'
      })
      this.queryConfig.forEach(item => {
        if (item.field === 'businessModuleName') {
          item.itemRender.options = data.results?.map(it => {
            return {
              label: it.businessName,
              value: it.businessName
            }
          }) || []
        }
      })
