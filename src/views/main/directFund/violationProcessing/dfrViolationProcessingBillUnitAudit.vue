            ...
            <template v-slot:toolbarSlots>
              <div class="table-toolbar-left">
                <div class="table-toolbar-left-title">
                  <span class="fn-inline">直达资金违规处理单确认</span>
                  <i class="fn-inline"></i>
                </div>
              </div>
            </template>
...
import { buttons1, curStatusButton, statusButtons } from './dfrViolationProcessingBillUnitAudit'
...
export default {
  ...
  methods: {
    ...
    bsToolbarClickEvent(obj, $this) {
      ...
      this.params.status = this.tabSelect === '1' ? '2' : '3'
      this.refresh()
    },
      ...
      switch (obj.code) {
        ...
        case 'toorbar_cancel':
          ...
          if (selectionRow2.length === 0) {
            this.$message.success('请选择至少一条任务取消确认')
            return
          }
    ...
    recallaudit(datas) {
      this.$confirm('此操作将取消确认选中数据, 是否继续?', '提示', {
    ...
    sendProc(datas) {
      this.$confirm('此操作将进行确认数据, 是否继续?', '提示', {
        confirmButtonText: '继续',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
