...
<script>
import getFormData from './budgetPayCapital.js'
...
export default {
  ...
  methods: {
    ...
    handleDetail(type, trackProCode, recDivCode) {
      let params = {
        reportCode: type,
    ...
    cellClick(obj, context, e) {
      ...
      switch (key) {
        ...
        case 'shapAmount':
        case 'xapAmount':
          this.handleDetail('zdzjzcmx_fzj', obj.row.code)
    ...
    cellStyle({ row, rowIndex, column }) {
      if (['sapAmount', 'shapAmount', 'xapAmount'].includes(column.property)) {
        return {
          color: '#4293F4',
