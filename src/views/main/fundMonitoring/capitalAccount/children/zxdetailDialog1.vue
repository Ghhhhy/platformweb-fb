...
import utils from '@/views/main/fundMonitoring/capitalAccount/specialSupervisionRegion/specialSupervisionRegionMethods.js'
  ...
  methods: {
    ...
    initQueryConfig() {
      ...
      this.queryConfig.forEach(query => {
        if (query.itemRender && (query.itemRender.name === '$vxeTree' || query.itemRender.name === '$formTreeInput')) {
    ...
    queryTableDatas() {
      ...
      console.log(params)
      HttpModule.getDetailTableDatas(params).then((res) => {
        if (res.code === '000000') {
