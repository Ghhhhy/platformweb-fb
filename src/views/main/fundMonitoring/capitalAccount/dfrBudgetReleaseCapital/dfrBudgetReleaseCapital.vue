...
export default {
  ...
  methods: {
    ...
    handleDetail(type, speTypeCode, obj) {
      if (this.isSx) {
        let params = {
        ...
        let condition = ''
        let column = obj.column.property
          ...
          switch (column) {
            ...
            case 'shbjfpaAmount':// 市级-分配本级-总金额
              if (this.$store.getters.isFuJian) {
                condition = ' substr(mof_div_code,7,3)=\'000\' '
              } else if (this.$store.getters.isNeiMeng) {
    ...
    cellClick(obj, context, e) {
      if (this.isSx) {
        ...
        switch (key) {
          case 'jOut':
            this.handleDetail('jOut', obj.row.speTypeCode, obj)
            this.detailTitle = '支出明细'
            break
          case 'sbbjfpAmount':
          case 'xbjfpAmount':
          case 'sbjfpAmount':
            this.handleDetail('sbbjfpAmount', obj.row.speTypeCode, obj)
        ...
        switch (key) {
          ...
          case 'shbjfpaAmount':
          case 'xyfpaAmount':
            this.handleDetail('zdzjxmmx_dfap', obj.row.code, obj)
            this.detailTitle = '直达资金项目明细'
        }
      }
    },
