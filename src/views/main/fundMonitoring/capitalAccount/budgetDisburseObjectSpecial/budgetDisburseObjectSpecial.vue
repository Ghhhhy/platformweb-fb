...
export default {
  ...
  methods: {
    cellClick(obj, context, e) {
      ...
      console.info(obj.column.property)
      if (isInvalidCellValue && obj.column.property !== 'proName') return
      ...
      switch (key) {
        case 'amountYszyap':
          ...
          break
        case 'amountZczyap':
          this.handleDetail('zxjdxmtz_zcmx', '1', obj.row.mofdivcode, obj.row.proCode, obj.row.proName, obj.row.agencyCode, obj.row.agencyName, obj.row.speTypeCode, obj.row.speTypeName, key)
    ...
    queryTableDatas() {
      ...
      HttpModule.xmSpecialPageQuery(param).then(res => {
        if (res.code === '000000') {
          if (res.data) {
