...
<script>
import { proconf } from './specialMultipleQuery'
import FilterTreeInput from '../benefitPeople/FilterTreeInput.vue'
import HttpModule from '@/api/frame/main/fundMonitoring/specialMultipleQuery.js'
...
export default {
  ...
  methods: {
    search(obj) {
      console.log(obj)
      this.speCode = obj.speCode
      this.speName = obj.speName
    ...
    onClickmethod(node) {
      ...
      if (node.label !== '全部') {
        this.mofDivCode = node.mofDivCode ? node.mofDivCode : ''
      } else {
        this.mofDivCode = ''
        this.mofDepCode = ''
        this.mofDepName = ''
      ...
      this.mofDepCode = node.mofDepCode
      this.mofDepName = node.mofDepName
      this.speCodes = node.speCode
      this.speNames = node.speName
      this.queryTableDatas()
    },
    getItem(code, data) {
      data.forEach(item => {
    ...
    queryTableDatas() {
      const param = {
        ...
        speCode: this.speCode,
        mofDivCode: this.mofDivCode,
        mofDepCode: this.mofDepCode,
        mofDepName: this.mofDepName,
        speCodes: this.speCodes,
        speNames: this.speNames
      }
    ...
    getLeftTreeData() {
      ...
      const param = {
        ...
        speCodes: this.speCodes,
        speNames: this.speNames,
        queryParm: this.treeFilterText.trim()
      }
      HttpModule.getTreeData(param).then(res => {
        if (res.code === '000000') {
