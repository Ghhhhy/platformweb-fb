...
export default {
  ...
  methods: {
    ...
    operationToolbarButtonClickEvent(obj, context, e) {
      ...
      switch (obj.code) {
        ...
        case 'confirm':
          ...
          this.$confirm('此操作将确认上报选中数据, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            this.updateLog(datas)
          }).catch(() => {
            this.$message({
              type: 'info',
              message: '已取消确认'
        ...
        case 'cancel':
          ...
          if (!isSb) {
            this.$message.info('请选择已确认且未上报的数据！')
            return
          }
          datas.forEach((item) => {
            item.confirmStatus = '0'
          })
          this.$confirm('此操作将撤销确认选中数据, 是否继续?', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
    ...
    queryTableDatas() {
      ...
      this.tableLoading1 = true
      HttpModule.queryLogs(param).then(res => {
    ...
    cellStyle({ row, rowIndex, column }) {
      if (['reportName'].includes(column.property)) {
        return {
          color: '#4293F4',
