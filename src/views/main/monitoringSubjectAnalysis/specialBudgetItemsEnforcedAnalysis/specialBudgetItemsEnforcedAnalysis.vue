...
<script>
import { proconf } from './specialBudgetItemsEnforcedAnalysis'
  ...
  methods: {
    ...
    queryTableDatas() {
      const param = {
        ...
        exePeriod: this.searchDataList.exePeriod,
        mofDivCodes: this.searchDataList.mofDivCodes_code__multiple,
        agencyCodes: this.searchDataList.agencyCodes_code__multiple,
        nationalPayRate: this.searchDataList.nationalPayRate
      }
      this.tableLoading = true
      HttpModule.getSpeItemsEnforcedAnalysisData(param).then(res => {
        this.tableLoading = false
        if (res.code === '000000') {
