<!--
 * @Author: 郭键鹏
 * @Date: 2023-11-23 08:43:24
 * @LastEditTime: 2023-11-23 09:06:44
...
export default {
  ...
  methods: {
    ...
    dialogClose() {
      this.$parent.totalDetailVisible = false
    },
    ...
    queryTableDatas() {
      ...
      this.$http.post('dfr-monitor-service/dfr/supervision/query', params).then((res) => {
        if (res.code === '000000') {
    ...
    showInfo() {
      ...
      let key = this.detailQueryParam.url
      this.getFormData = require(`../js/${key}.js`).default
      this.tableColumnsConfig = this.getFormData('basicInfo', 'tableColumnsConfig')
      this.queryTableDatas()
    },
    ConfigTable() {
      if (
