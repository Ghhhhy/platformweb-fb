...
export default {
  ...
  methods: {
    ...
    onSearchClick() {
      ...
      if (!this.month) {
        this.$XModal.message({ status: 'info', message: '请先选择下达批次' })
        return false
      }
      if (!this.dataType) {
    ...
    onInputTableTreeNodeClick({ node }) {
      ...
      if (node.name.includes('单位明细')) {
        if (this.agencyTreeQueryparams.levelNo !== 3) {
          this.agencyTreeQueryparams.levelNo = 3
        ...
        if (this.agencyTreeQueryparams.levelNo !== 2) {
          this.agencyTreeQueryparams.levelNo = 2
          this.currentTableNode = node
          this.checkedAgencys = {
            agencyIds: '',
            agencyCodes: '',
      ...
      } else if (node.name.includes('处室汇总')) {
        if (this.agencyTreeQueryparams.levelNo !== 1) {
          this.agencyTreeQueryparams.levelNo = 1
          this.currentTableNode = node
          this.checkedAgencys = {
            agencyIds: '',
            agencyCodes: '',
