      ...
      <template v-slot:mainForm>
        <div style="margin-bottom: 10px;margin-top: 10px;">
          <div style="float: right;">
            ...
            <vxe-button status="primary" content="查询" @click="onSearchClick" />
          </div>
  ...
  data() {
    return {
      ...
      reportViewArray: [
        ...
        {
          id: 'a7cf2fe0924711ed99e6f52ca1156e1f',
          name: '会议费细化'
        }
      ],
      isleaf: true,
  ...
  methods: {
    ...
    validateExcelData() {
      ...
      if (totalConferenceFee !== parseFloat((firstConferenceFee + secondConferenceFee + thirdConferenceFee + fourthConferenceFee).toFixed(2))) {
        flag = true
        messageArr.push('一类会议，二类会议，三类会议，四类会议之和不等于会议费合计，无法提交。')
      }
      return {
        flag: flag,
    ...
    submit() {
      ...
      if (errorObj?.flag) {
        ...
        self.$http.post('/bisBudget/fillReport/updateConferenceFee', {
          ...
          f1: parseFloat(this.updateData[2]).toFixed(2),
          f2: parseFloat(this.updateData[3]).toFixed(2),
          f3: parseFloat(this.updateData[4]).toFixed(2),
          f4: parseFloat(this.updateData[5]).toFixed(2)
        }).then(res => {
    ...
    commit() {
      ...
      if (this.$store.getters.getMenuParams5.reportStatus === '2') {
        ...
        this.commitLoading = true
        self.$XModal.confirm('您确定要将选中单位的会议费明细数据提交吗?').then(type => {
          if (type === 'confirm') {
            self.$http.post('/bisBudget/fillReport/conferenceFeeBatchCommit', {
              agencyCodes: self.checkedAgencys.agencyCodes,
              feeStatus: +self.$store.getters.getMenuParams5.reportStatus + 1
            }).then(res => {
              if (res && res.code === '100000') {
                this.$refs.agencyTree.initTreeData(true)
    ...
    cancel() {
      ...
      if (+self.params5.reportStatus === 2) {
        ...
        this.commitLoading = true
        self.$XModal.confirm('您确定要将选中单位的会议费明细数据撤销提交吗?').then(type => {
          if (type === 'confirm') {
            self.$http.post('/bisBudget/fillReport/conferenceFeeBatchCommit', {
              agencyCodes: self.checkedAgencys.agencyCodes,
              feeStatus: +self.$store.getters.getMenuParams5.reportStatus - 1
    ...
    back() {
      ...
      if (this.$store.getters.getMenuParams5.reportStatus === '2') {
        this.commitLoading = true
        self.$XModal.confirm('您确定要将' + self.checkedAgencys.agency.name + '单位的会议费明细数据退回吗?').then(type => {
          if (type === 'confirm') {
            self.$http.post('/bisBudget/fillReport/conferenceFeeCommit', {
              agencyCode: self.checkedAgencys.agencyCodes,
              feeStatus: +self.$store.getters.getMenuParams5.reportStatus - 3
            }).then(res => {
              if (res && res.code === '100000') {
                self.$refs.agencyTree.initTreeData(true)
                self.commitLoading = false
        ...
        this.commitLoading = true
        self.$XModal.confirm('您确定要将选中单位的会议费明细数据退回吗?').then(type => {
          if (type === 'confirm') {
            self.$http.post('/bisBudget/fillReport/conferenceFeeBatchCommit', {
              agencyCodes: self.checkedAgencys.agencyCodes,
              feeStatus: +self.$store.getters.getMenuParams5.reportStatus - 3
      ...
      if (this.$store.getters.getMenuParams5.reportStatus === '2') {
        ...
        self.$XModal.confirm('您确定要将' + self.checkedAgencys.agency.name + '单位的会议费明细数据退回吗?').then(type => {
          if (type === 'confirm') {
            self.$http.post('/bisBudget/fillReport/conferenceFeeCommit', {
              agencyCode: self.checkedAgencys.agencyCodes,
              feeStatus: +self.$store.getters.getMenuParams5.reportStatus - 5
            }).then(res => {
              if (res && res.code === '100000') {
                self.$refs.agencyTree.initTreeData(true)
                self.commitLoading = false
    ...
    onNodeCheckChange() {
      ...
      if (agencies.length > 0) {
        ...
        this.currentTableNode = this.reportViewArray[1]
        this.setReportableParams(this.currentTableNode)
        setTimeout(() => {
          this.delarLoading = false
        }, 500)
    ...
    onSearchClick() {
      ...
      if (this.$store.getters.getMenuParams5.reportStatus === '2') {
        ...
        if (this.$refs.agencyTree.getCheckedNodes().length === 0) {
          this.$XModal.message({ status: 'info', message: '请先选择预算单位' })
          return false
        }
      }
