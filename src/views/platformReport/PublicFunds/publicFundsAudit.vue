    ...
    <BsMainFormListLayout :left-visible.sync="leftVisible">
      ...
      <template v-slot:mainForm>
        <div style="margin-bottom: 10px;margin-top: 10px;">
          <div v-if="leftTreeStatus === ''" style="float: right;">
            <!-- <vxe-input
              v-model="month"
              placeholder="下达月份"
          ...
          <div style="float: right;">
            ...
            <vxe-button v-if="leftTreeStatus === wshStatus" status="primary" :loading="commitLoading" :content="params5.reportStatus === '2' ? '提交' : '审核'" @click="commit" />
            <vxe-button v-if="leftTreeStatus === shStatus" status="primary" :loading="commitLoading" :content="params5.reportStatus === '2' ? '撤销提交' : '撤回'" @click="cancel" />
            <vxe-button v-if="leftTreeStatus === wshStatus && params5.reportStatus !== '2'" status="primary" :loading="commitLoading" content="退回" @click="back" />
            <vxe-button v-if="leftTreeStatus === wshStatus && params5.reportStatus === '6'" status="primary" :loading="commitLoading" content="退回第一岗" @click="backFirst" />
            <vxe-button v-if="params5.reportStatus === '4' && leftTreeStatus === wsbStatus" status="primary" @click="submit">保存</vxe-button>
            <vxe-button v-if="params5.reportStatus === '4' && leftTreeStatus === wsbStatus" status="primary" :loading="commitLoading" content="提交" @click="agencyCommit" />
...
export default {
  ...
  data() {
    return {
      ...
      wsbStatus: this.$store.getters.getMenuParams5.reportStatus !== '2' ? '(' + this.$store.getters.getMenuParams5.wtjStatus.replaceAll('-', ',') + ')' : '-',
      ...
      agencyTreeConfig: {
        ...
        isleaf: this.$store.getters.getMenuParams5.reportStatus === '2' ? 1 : 0, // 指定节点是否为叶子节点，仅在指定了 lazy 属性的情况下生效
        levelno: -1, // 可选层级
        ...
        readonly: true,
        clearable: true,
        multiple: this.$store.getters.getMenuParams5.reportStatus !== '2'
      },
  ...
  methods: {
    ...
    onTapPanelTabClick(obj) {
      ...
      if (obj.label === '未上报' && this.params5.reportStatus === '4') {
        this.agencyTreeConfig.multiple = false
      } else if (this.params5.reportStatus === '2') {
        this.agencyTreeConfig.multiple = false
      } else {
    ...
    getSubmitData() {
      ...
      self.addData = []
      self.updateData = []
      if (self.currentTableNode.id && !this.params5.reportType.endsWith('Dept') && !this.params5.reportType.endsWith('Ywcs')) {
    ...
    commit() {
      ...
      if (this.$store.getters.getMenuParams5.reportStatus === '2') {
        ...
        this.commitLoading = true
        self.$XModal.confirm('您确定要将选中单位的报表:' + Array.from(new Set(this.inputTableTreeData.map(v => v.name))).toString() + '数据提交吗?').then(type => {
          if (type === 'confirm') {
            self.$http.post('/bisBudget/fillReport/reportBatchCommit', {
              agencyIds: self.checkedAgencys.agencyIds,
              status: +self.$store.getters.getMenuParams5.reportStatus + 1,
              issuedType: this.params5.IssuedType
            }).then(res => {
              if (res && res.code === '100000') {
                ...
                this.commitLoading = false
                this.currentTableNode = {}
                self.$XModal.message({ status: 'success', message: '审核成功' })
              } else {
                self.$XModal.message({ status: 'error', message: `错误：[${res.code}-${res.msg}]` })
                this.commitLoading = false
              }
    ...
    cancel() {
      ...
      if (+self.params5.reportStatus === 2) {
        ...
        let agency = self.checkedAgencys.agency.filter(v => +v.agencyStatus === 7)
        if (agency.length > 0) {
          self.$XModal.message({ status: 'warning', message: '' + Array.from(new Set(agency.map(v => v.code))).toString() + '单位数据业务处已审核，无法取消审核' })
          return false
        }
      }
      if (+self.params5.reportStatus === 2) {
        ...
        this.commitLoading = true
        self.$XModal.confirm('您确定要将选中单位的报表数据撤销提交吗?').then(type => {
          if (type === 'confirm') {
            self.$http.post('/bisBudget/fillReport/reportBatchCommit', {
              agencyIds: self.checkedAgencys.agencyIds,
              status: +self.$store.getters.getMenuParams5.reportStatus - 1,
    ...
    back() {
      ...
      if (+self.params5.reportStatus === 2) {
        this.commitLoading = true
        self.$XModal.confirm('您确定要将' + self.checkedAgencys.agency.name + '单位的报表数据退回吗?').then(type => {
          if (type === 'confirm') {
            self.$http.post('/bisBudget/fillReport/reportCommit', {
              agencyId: self.checkedAgencys.agency.id,
              status: +self.$store.getters.getMenuParams5.reportStatus - 3,
              IssuedType: this.params5.IssuedType
            }).then(res => {
              if (res && res.code === '100000') {
                self.$refs.agencyTree.initTreeData(true)
                self.commitLoading = false
                self.currentTableNode = {}
                self.$XModal.message({ status: 'success', message: '退回成功' })
              } else {
                self.$XModal.message({ status: 'error', message: `错误：[${res.code}-${res.msg}]` })
                self.commitLoading = false
              }
        ...
        this.commitLoading = true
        self.$XModal.confirm('您确定要将选中单位的报表数据退回吗?').then(type => {
          if (type === 'confirm') {
            self.$http.post('/bisBudget/fillReport/reportBatchCommit', {
              agencyIds: self.checkedAgencys.agencyIds,
              status: +self.$store.getters.getMenuParams5.reportStatus - 3,
    ...
    backFirst() {
      ...
      if (+self.params5.reportStatus === 2) {
        this.commitLoading = true
        self.$XModal.confirm('您确定要将' + self.checkedAgencys.agency.name + '单位的报表数据退回第一岗吗?').then(type => {
          if (type === 'confirm') {
            self.$http.post('/bisBudget/fillReport/reportCommit', {
              agencyId: self.checkedAgencys.agency.id,
              status: +self.$store.getters.getMenuParams5.reportStatus - 5,
              IssuedType: this.params5.IssuedType
            }).then(res => {
              if (res && res.code === '100000') {
                self.$refs.agencyTree.initTreeData(true)
                self.commitLoading = false
                self.currentTableNode = {}
                self.$XModal.message({ status: 'success', message: '退回第一岗成功' })
              } else {
                self.$XModal.message({ status: 'error', message: `错误：[${res.code}-${res.msg}]` })
                self.commitLoading = false
              }
        ...
        this.commitLoading = true
        self.$XModal.confirm('您确定要将选中单位的报表数据退回第一岗吗?').then(type => {
          if (type === 'confirm') {
            self.$http.post('/bisBudget/fillReport/reportBatchCommit', {
              agencyIds: self.checkedAgencys.agencyIds,
              status: +self.$store.getters.getMenuParams5.reportStatus - 5,
    ...
    onTreeLoaded({ treeData, tree, currentNodeKey, curSelectNode }, bstree) {
      ...
      if (id) {
        ...
        if (this.$store.getters.getMenuParams5.reportStatus === '2' || (this.$store.getters.getMenuParams5.reportStatus === '4' && this.leftTreeStatus === this.wsbStatus)) {
    ...
    onAgencyTreeNodeClick({ node }) {
      if (node && (this.$store.getters.getMenuParams5.reportStatus === '2' || (this.$store.getters.getMenuParams5.reportStatus === '4' && this.leftTreeStatus === this.wsbStatus))) {
        let agency = node
    ...
    agencyDeptChange(node) {
      // 单位部门报表切换
      if (this.$store.getters.getMenuParams5.reportStatus === '4') {
        if (this.leftTreeStatus === this.wsbStatus) {
          if (this.isleaf) {
            if (!node.isleaf) {
      ...
      } else if (this.$store.getters.getMenuParams5.reportStatus === '6') {
        if (!this.params5.reportType.endsWith('Ywcs')) {
          this.params5.reportType += 'Ywcs'
        }
        this.getDeptReportData(this.params5.reportType)
      } else {
        this.getReportData(this.params5.reportType)
      }
      // if (this.checkedAgencys.agencyCodes === 'root') {
      //   if (this.params5.reportType.endsWith('Dept')) {
      //     this.params5.reportType = this.params5.reportType.substring(0, this.params5.reportType.length - 4) + 'Ywcs'
      //   } else if (!this.params5.reportType.endsWith('Dept') && !this.params5.reportType.endsWith('Ywcs')) {
      //     this.params5.reportType += 'Ywcs'
      //   }
      //   this.getDeptReportData(this.params5.reportType)
      // } else {
    ...
    onSearchClick() {
      ...
      this.$http.post('/bisBudget/fillReport/getIssuedMonth', {
        reportId: !this.params5.reportType.endsWith('Dept') && !this.params5.reportType.endsWith('Ywcs') ? this.currentTableNode.id : this.currentTableNode.reportvname,
      ...
      //   })
      // }
      if (this.params5.reportStatus === '4' && !this.params5.reportType.endsWith('Ywcs') && !this.params5.reportType.endsWith('Dept') && this.leftTreeStatus === this.wsbStatus) {
        this.editable = true
      } else {
        this.editable = false
      }
    ...
    getDeptReportData(reportType) {
      ...
      self.$http.post('bisBudget/fillReport/queryPublicFundsReportTree', param).then((res) => {
        ...
        self.resolveResult(data => {
          if (this.leftTreeStatus === this.shStatus || this.leftTreeStatus === this.wshStatus) {
            self.inputTableTreeData = data.filter(v => !v.name.includes('未上报单位'))
          } else if (this.params5.reportStatus === '4' && this.leftTreeStatus === this.wsbStatus) {
            self.inputTableTreeData = data.filter(v => v.name.includes('未上报单位'))
          } else {
            self.inputTableTreeData = data
          }
    ...
    setReportableParams(inputReport, month) {
      this.reportParams = {
        ...
        paramsObj: {
          ...
          month: month,
          wsbStatus: this.$store.getters.getMenuParams5.wtjStatus ? '(' + this.$store.getters.getMenuParams5.wtjStatus.replaceAll('-', ',') + ')' : ''
        }
      }
    },
    getreportParams() {
