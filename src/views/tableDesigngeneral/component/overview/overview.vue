    ...
    <div class="height-all pdl10 pdr10">
      <div class="option-line-group">
        <div class="fn-inline option-line-group-btn pdl0 pdr0">
          <div class="fn-inline">
            <vxe-button v-if="type === 'table'" round size="mini" icon="el-icon-circle-plus-outline" @click="insertRow">新增行</vxe-button>
            <vxe-button
              round
              size="mini"
            ...
            >校验</vxe-button>
            <vxe-switch v-model="type" open-label="表单" open-value="form" close-label="表格" close-value="table" />
          </div>
      ...
      <div class="main-modulebox-tabcon ofa" style="max-height:calc(100% - 44px);padding: 0px 10px 10px 10px;background: #dddfe61f;border: solid 1px #dddfe6;overflow: auto;box-sizing: border-box;">
        <div>
          <!-- <BsTitle type="left">
            <template slot="default">{{ type === 'form' ? '表单项预览' : '表格列预览' }}</template>
          </BsTitle>
          -->
          <div style="height: 380px">
            <BsTable
              v-if="type === 'table' || type === 'editTable'"
              ref="bsTableRef"
