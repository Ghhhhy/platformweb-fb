...
import Encrypt from './httpSecret.js'

let globalGatewayAgentConf = window.gloableToolFn.serverGatewayMap
...
let isSecret = globalGatewayAgentConf.isOpen ? globalGatewayAgentConf.isSecret : globalGatewayAgentConf.isMicroSecret
...
export function get(url, params, Origin, contentType) {
  let param = Encrypt.encryHttp({ isSecret, ajaxType: 'get', params })
...
// 封装axios的post请求
export function post(url, data = {}, Origin, contentType, source) {
  ...
  let params = source === 'login' ? data : Encrypt.encryHttp({ isSecret, ajaxType: 'post', params: data })
  return new Promise((resolve, reject) => {
    axios
      .post(globalGatewayAgent(url, Origin), params)
      .then(response => {
...
export function put(url, data = {}, Origin, contentType) {
  ...
  return new Promise((resolve, reject) => {
    axios
      .put(globalGatewayAgent(url, Origin), Encrypt.encryHttp({ isSecret, ajaxType: 'put', params: data }))
      .then(response => {
        // console.log('post', '\n', url, 'response', response, '\n', data, '\n', response)
        resolve(response.data)
      })
...
export function del(url, params, Origin, contentType, dataType) {
  ...
  let paramsData = dataType ? { params: params } : { data: params }
  let data = Encrypt.encryHttp({ isSecret, ajaxType: 'delete', params: paramsData, dataType }, contentType)
  return new Promise((resolve, reject) => {
    axios
      .delete(globalGatewayAgent(url, Origin), data)
      .then(response => {
        // console.log('post', '\n', url, 'response', response, '\n', params, '\n', response)
        resolve(response.data)
      })
