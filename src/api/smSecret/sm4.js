/*
 * @Author: 轻语
 * @Date: 2021-07-12 09:53:11
 * @LastEditors: 轻语
 * @LastEditTime: 2021-07-12 18:32:45
 * @Description:
 */
...
var SM4Util = {
   ...
   * @returns {*}
   */
  encryptECB: function(parameters, isRecursive) {
    if (!parameters) {
      return parameters
    }
    ...
    if (Array.isArray(curParam)) { // 是数组
      curParam.forEach((row, index) => {
        if (!row && row != 0) { // 非空的不处理
          return
        }
        if (isPlainObject(row) || Array.isArray(row)) { // 对象和数组需要递归
          SM4Util.encryptECB(row, true)
        } else { // 其他直接加密
          curParam[index] = SM4Util.encryptData_ECB(row + '')
        }
      })
      // $.each(curParam, function(index, row) {

      // })
    } else if (isPlainObject(curParam)) { // 对象
      for (var key in curParam) {
        if (!curParam[key] && curParam[key] != 0) { // 非空的不处理
          continue
        }
        if (isPlainObject(curParam[key]) || Array.isArray(curParam[key])) { // 对象和数组需要递归
          SM4Util.encryptECB(curParam[key], true)
        } else {
          curParam[key] = SM4Util.encryptData_ECB(curParam[key] + '')
        }
      }
    }
    return curParam
