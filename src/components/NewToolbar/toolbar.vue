  ...
  <div class="boss-slect__toolbar">
    <ul ref="toolbarContent" class="boss-toolbar__content">
      ...
      >
              ...
              <vxe-button ref="btnMore" content="更多">
                <template v-slot:dropdowns>
                  <div
                    ...
                    :key="index"
                  >
                    <vxe-button v-if="!item.options || (item.options && !item.options.length)" v-preventClick="preventClick" type="text" :content="item.label" @click.native.stop="onClickBtn(item)" />
                    <div
                      v-for="(it, idx) in item.options"
                      v-else
                      :key="idx"
                    >
                      <vxe-button v-preventClick="preventClick" type="text" :content="it.label" @click.native.stop="onClickBtn(item)" />
                    </div>
                  </div>
                </template>
              </vxe-button>
        ...
        <div style="height:30px;border-right:1px solid #edf2aa;margin:0 16px">
        </div>
        <!--后置按钮插槽组-->
        ...
        >
          ...
          <i :class="isPack ? 'ri-search-line' : 'ri-search-eye-line'" class="fn-inline"></i>
...
export default {
  ...
  methods: {
      ...
      list.forEach((item) => {
        ...
        if (item.type === 'select') {
          ...
          optionsCp.forEach((option) => {
            ...
            if (num) {
              option.num = num
              option.labelNum = option.labelZero = `${item.label} - ${option.label} (${num})`
            } else {
              option.num = 0
              option.labelNum = `${item.label} - ${option.label}`
              option.labelZero = `${item.label} - ${option.label} (0)`
            }
          })
    ...
    resize() {
      ...
      const queryBtn = this.isHideQuery ? 0 : 120
    ...
    dealRbuttons(overage) {
      let self = this
      const btnNomalW =
        (this.$refs.btnNomal && this.$refs.btnNomal.offsetWidth - (this.hasMoreBtn ? 82 : 0)) || 0 // 右侧按钮组，去除更多剩余按钮的宽度
      const preSlotW =
        (this.$refs.extendPre && this.$refs.extendPre.offsetWidth) || 0
      const lastSlotW =
        (this.$refs.extendLast && this.$refs.extendLast.offsetWidth) || 0
      ...
      if (btnNomalW >= overage) {
        ...
        if (this.rButtons.length) {
          ...
          let newOverage = btnNomalW - 82 - preSlotW - lastSlotW// 右侧按钮组,去除更多剩余空间
          this.$nextTick(() => {
  ...
  watch: {
    ...
    isHideQuery(newValue) {
      this.$emit('update:isHideQuery', newValue)
    },
    isOpen(val) {
