...
export class Import {
  constructor(gloabelConfig = {}) {
    ...
    this.importGloableConf = {
      valiRules: {}
    }
    this.importHistory = [] // 导出历史记录
  }
  importExcel(billguid, conf, cb) {
    this.billguid = billguid
    this.importGloableConf = Object.assign(this.importGloableConf, conf)
    this.callback = cb
    this.selectFile(cb)
  }
  selectFile() {
  ...
  getParams(file, billguid) {
    let param = new FormData() // 创建form对象
    param.append('file', file)
    param.append('filename', file.name)
    param.append('appid', 'pay')
    param.append('billguid', billguid || '')
    param.append('doctype', 'excel')
    param.append('year', userInfo.year)
    param.append('province', userInfo.province)
    param.append('userguid', userInfo.guid)
    return param
  }
  readWorkbookFromLocalFile(files) {
    ...
    console.log('this.billguid', this.billguid)
    const params = this.getParams(files[0], this.billguid)
    post('fileservice/v2/upload', params, null, 'multipart/form-data', 'openapi').then(res => {
      if (res.rscode === '100000') {
        ...
        reader.onload = function (e) {
          ...
          self.importHistory.push({ import: workbook })
          self.importHistory = self.importHistory.reverse().slice(0, 20).reverse()
          let excelJson = self.XlsxTool.utils.sheet_to_json(workbook.Sheets[workbook.SheetNames[0]])
          self.callback(excelJson)
        }
