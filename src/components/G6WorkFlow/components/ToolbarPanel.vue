<!--  工作流toolbar   Author:Titans@2396757591@qq.com -->
<template>
  <div class="toolbar wfd-toolbar">
    <div class="toolbar-left">
      <template v-for="(item,index) in toolbarList">
        <vxe-tooltip v-if="!item.separator" :key="index" :content="i18n[item.n118n]">
          <span class="command" :data-command="item.command">
            <span :class="item.ico"></span>
          </span>
        </vxe-tooltip>
        <span v-if="item.separator" :key="index" class="separator"></span>
      </template>
    </div>
