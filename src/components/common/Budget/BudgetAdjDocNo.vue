<template>
  <div style="padding-left: 20px;">
    <div v-show="custom" style="margin-top: 10px">
      <el-row :gutter="10">
        <el-col class="flag" :span="0.1"><span style="color: red">*</span></el-col>
        <el-col :span="6">
          <BsTreeInputTest
            ref="bgtdocno"
        ...
        <el-col :span="2"><span class="index_items" @click="standardNo">标准文号</span></el-col>
      </el-row>
    </div>
    <div v-show="!custom" style="margin-top: 10px">
      <el-row :gutter="5">
    ...
    getBgtdocnoValue() {
      ...
      if (this.custom) {
        if (this.curData.defaultTreedocnoValue) {
          ...
          docnoValue.adj_doc_no = docnoStr.split('##')[2].trim()
          docnoValue.adj_doc_no_id = docnoStr.split('##')[0].trim()
          docnoValue.adj_doc_no_code = docnoStr.split('##')[1].trim()
          docnoValue.adj_doc_no_name = docnoStr.split('##')[2].trim()
        }
      } else {
        ...
        //   return
        // }
        docnoValue.adj_doc_no = (!this.curData.docno0Value ? '' : this.curData.docno0Value) + (!this.curData.docno1Value ? '' : this.curData.docno1Value) + this.curData.year + this.docno + '号'
        docnoValue.adj_doc_no_name = (!this.curData.docno0Value ? '' : this.curData.docno0Value) + (!this.curData.docno1Value ? '' : this.curData.docno1Value) + this.curData.year + this.docno + '号'
      }
      return docnoValue
    },
    getOaNo() {
