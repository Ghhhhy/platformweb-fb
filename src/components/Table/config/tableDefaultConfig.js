...
import $ToolFn from '../tool/util.js'
const defaultGloabalUtils = {
    ...
    return columns.map((column, columnIndex) => {
      ...
      if (columnIndex === 0) {
        ...
        try {
          ...
          if (result.length) {
            result = $ToolFn.reduceCalc(result, $ToolFn.accurateJiafa)
  ...
  calculateFooter_totalAll({ columns, data, $table, $grid }) {
    ...
    if (!item || !pager) {
      return defaultGloabalUtils.calculateFooter_total({ columns, data, $table, $grid })
    } else {
      return columns.map((column, columnIndex) => {
        if (columnIndex === 0) {
          return '总计'
...
const cellRenderers = {
  ...
  $optionSort: {
    ...
    renderEdit(h, cellRender, { $columnIndex, $rowIndex, items, column, data, row, $seq, columnIndex, isEdit, level, rowIndex, seq }, { $excel, $grid, $table, $type }) {
      let self = $grid.$parent
      // <a class="optionRow" href={row.link}>{'操作' + row[column.property]}</a>
      return [
        <div class="row-dragsort-option row-option-group">
    ...
    renderCell(h, cellRender, { $columnIndex, $rowIndex, items, column, data, row, $seq, columnIndex, isEdit, level, rowIndex, seq }, { $excel, $grid, $table, $type }) {
      let self = $grid.$parent
      // <a class="optionRow" href={row.link}>{'操作' + row[column.property]}</a>
      return [
        <div class="row-dragsort-option row-option-group">
    ...
    return footerData.map((row) => {
      return row.map((item, index) => {
        if ((columns[index].own.combinedType + '').split(',').filter((item) => { return !!(item + '').replace(/null|undefined|true|false/ig, '') }).length) {
          let result = 0
          let render = columns[index].cellRender || columns[index].editRender
          let props = render.props || {}
          if ((columns[index].own.combinedType + '').indexOf('integer') >= 0 || props.type === 'integer') {
            result = parseInt(item)
          } else if ((columns[index].own.combinedType + '').indexOf('float') >= 0) {
            result = parseFloat(item).toFixed(2)
          } else if (render && (render.name === '$vxeMoney' || render.name === '$vxeCalculate')) {
            // let showZero = bsTable.showZero
            let viewDigits = props.digits || bsTable.curSelectMoneyUnitOption.viewDigits
            result = bsTable.transToNumber(item, viewDigits)
            if (bsTable.toolbarConfigInCopy.moneyInputSwich) {
              result = bsTable.transToNumber(bsTable.accurateChuFa(result, moneyUnit), viewDigits)
            }
            result = bsTable.transToNumberLocalString(result, viewDigits)
          } else {
            result = item
          }
          return result
