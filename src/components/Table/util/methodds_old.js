...
/* eslint-disable no-new-func no-useless-call no-eval */
...
const sortMethods = {
  arrSortGloabal({ $table, data, column, property, order, rTypeSuffix }, type) {
    // 通用 排序
...
const initMethods = {
  ...
  initTableConlums() {
    ...
    const tableColumnsConfig = (this.tableGlobalConfig.dragColumnsConfig && this.tableGlobalConfig.dragColumnsConfig.cacheStatus) ? this.initCacheColumn() : this.tableColumnsConfig
  ...
  initScrollLoadConfig() {
    this.scrollLoadConfigCp = Object.assign({}, this.scrollLoadConfigCp, this.scrollLoadConfig)
  },
  ...
  initToolbar() {
    ...
    if (this.getbasicDataType(this.toolbarConfig) === 'Boolean') {
      ...
      if (this.toolbarConfig.visible === false) {
        this.toolbarConfigIn = undefined
        this.toolbarConfigInCopy = this.toolbarConfigInCp
      } else {
  ...
  initTableData_old(data) { // 初始化表格数据
    // 执行 顺序， 列计算-->分级汇总-->跨单元格计算-->单元格约束
    // 生成映射
    let self = this
    this.initCalculateConstraintConfig()
    let { rowformulaConfig, rowCodeFormulaConfig } = this.calculateConstraintConfigIn
    // 初始化表格数据
    this.selection = []
    if (this.$refs.xGrid) {
      this.$refs.xGrid.$refs.xTable.tableSourceData = []
    ...
    let calclataResult = this.addMissingFieldsAndCalcColFormula(data || this.tableData)
    // if (this.isEmptyObj(getDataConfig)) {
    //   this.performGetData()
    // }
    if (this.gradedSummaryFields && this.gradedSummaryFields.length) {
      calclataResult = this.performGradedSummaryData(calclataResult, this.gradedSummaryFields)
    }
    calclataResult = this.reductionRowFormula(rowformulaConfig, calclataResult, this.id)
    calclataResult = this.reductionRowCodeFormula(rowCodeFormulaConfig, calclataResult, this.id)
    let tableDataIn = calclataResult
    if (this.unidirectionalData) {
      tableDataIn = this.deepCopy(calclataResult)
    }
    this.tableDataMap = this.generatetTableDataMap(tableDataIn, this.rowId)
    this.tableDataIn = tableDataIn
    this.$nextTick().then(() => {
      if (self.$refs.xGrid) {
        self.$refs.xGrid.reloadData(self.tableDataIn).then(() => {
          self.$refs.xGrid.clearFilter().then(() => {
  ...
  initTableData(data) { // 初始化表格数据
    ...
    return new Promise((resolve, reject) => {
      this.rePerformAllCalcAndConstraintByData(data, true).then(({ dataCodeRowMap, dataCodeRowArr, afterCalcData }) => {
        if (self.unidirectionalData) {
          let { calcAndConstraintItemCodeField } = this.calculateConstraintConfigIn
          let { dataCodeRowMap } = self.recursiveDataCodeRowMap(data, calcAndConstraintItemCodeField)
...
const tableEvent = {
  ...
  editClosed(obj) {
    ...
    this.rePerformRowCalcAndConstraintSingle(obj).then((row) => {
      this.$emit('editClosed', Object.assign(obj, { row: row }), this, this.$refs.xGrid)
      this.$emit('ProxyEvent', 'editClosed', obj, this, this.$refs.xGrid)
    })
  }, //
...
const calculateTool = {
        ...
        if (colFormulaMapArr.length) {
          for (let cmai = 0; cmai < colFormulaMapArr.length; cmai++) {
            ...
            if (rowsKey.length) {
              ...
              hasCalcColFormulaMap[item] = calcResult === Infinity || calcResult === -Infinity || isNaN(calcResult) ? '' : calcResult
              // 列单独配置元和万元单位
        ...
        if (rowFormulaMapArr.length) {
          for (let cmai = 0; cmai < rowFormulaMapArr.length; cmai++) {
            ...
            if (rowsKey.length) {
              ...
              let calcResult = eval(formula)
              hasCalcrowFormulaMap[item] = calcResult === Infinity || calcResult === -Infinity || isNaN(calcResult) ? '' : calcResult
  ...
  reductionRowCodeFormula(rowCodeFormulaMap, data, tableId) {
    ...
    let self = this
    const { dataCodeRowMap } = this.reverDataCodeRowMap(data, this.calculateConstraintConfigIn.calcAndConstraintItemCodeField)
        ...
        if (rowCodeFormulaMapArr.length) {
          for (let cmai = 0; cmai < rowCodeFormulaMapArr.length; cmai++) {
            ...
            if (rowsKey.length) {
              ...
              let calcResult = eval(formula)
              hasCalcrowCodeFormulaMap[item] = calcResult === Infinity || calcResult === -Infinity || isNaN(calcResult) ? '' : calcResult
    ...
    columns.forEach((conlum, index) => {
      if (Array.isArray(conlum.children) && conlum.children.length) {
        ...
        if (conlum.field) {
          ...
          if ((conlum.combinedType && conlum.combinedType.indexOf('needGradedSummary') >= 0) || conlum.needResponsive) {
  ...
  generateCalcColFormulaMap(columns) {
    ...
    self.gradedSummaryFields = obj.gradedSummaryFields
  ...
  performTableDataCalculate({ row, rowIndex, $rowIndex, column, columnIndex, $columnIndex }) { // 行改变事件执行单元格计算并刷新视图
    // 执行顺序： 取数-->行内计算-->分级汇总-->父级行内计算-->单元格交叉计算(涉及的行计算)-->行约束-->单元格约束
    let self = this
    // let formColCalList = this.getFormColCalBack(this.id + column.property + row.itemcode)
    const { property } = column
    return new Promise((resolve, reject) => {
      row = self.reductionColFormula(self.calculateConstraintConfigIn.colFormulaConfig, row, property)
      // self.$refs.xGrid.reloadRow([row]).then((rownew) => {
      // if (Object.keys(self.calculateConstraintConfigIn.rowformulaConfig).length) {
      if (self.gradedSummaryFields.length) {
        let gradedSummaryRow = null
        while (this.tableDataMap[row['nest__pid']]) {
          gradedSummaryRow = this.tableDataMap[row['nest__pid']]
        }
        if (gradedSummaryRow) {
          self.performGradedSummarySingData(gradedSummaryRow, self.gradedSummaryFields)
        }
      }
...
const calcAndConstraint = { // 计算约束部分
    ...
    return new Promise((resolve, reject) => {
      try {
        if (Array.isArray && data.length) {
          let { colFormulaConfig, calcAndConstraintItemCodeField, rowCodeFormulaConfig, getDataRenderMap, gradedSummaryFields, enabled } = self.calculateConstraintConfigIn
          if (enabled) {
            let { dataCodeRowMap, dataCodeRowArr } = self.recursiveDataCodeRowMap(data, calcAndConstraintItemCodeField)
            self.performGetDataAll({ data, dataCodeRowMap, dataCodeRowArr, getDataRenderMap, tableId: self.id }) // 渲染取数
            self.performColFormulaAll({ data, dataCodeRowMap, dataCodeRowArr, colFormulaConfig, tableId: self.id, calcColType: 'lastLevel' }) // 行内计算
            self.performGradedSummaryAll({ data, dataCodeRowMap, dataCodeRowArr, gradedSummaryFields, tableId: self.id }) // 分级汇总
            self.performColFormulaAll({ data, dataCodeRowMap, dataCodeRowArr, colFormulaConfig, tableId: self.id, calcColType: 'parent' }) // 父级行内计算
            self.performRowCodeFormula({ data, dataCodeRowMap, dataCodeRowArr, rowCodeFormulaConfig, tableId: self.id }) // 单元格交叉计算
            // self.performRowConstraintFormula({ data, dataCodeRowMap,dataCodeRowArr, rowConstraintConfig, tableId: self.id }) // 行内约束
            // self.performCellConstraintFormula({ data, dataCodeRowMap,dataCodeRowArr, cellConstraintConfig, tableId: self.id }) // 单元格约束
            resolve({
              dataCodeRowMap,
              dataCodeRowArr,
              afterCalcData: data
  ...
  rePerformRowCalcAndConstraintSingle({ row, data, column }) { // 重新计算和生成单条约束数据
    ...
    return new Promise((resolve, reject) => {
      try {
        let { colFormulaConfig, calcAndConstraintItemCodeField, rowCodeFormulaConfig, gradedSummaryFields, enabled } = self.calculateConstraintConfigIn
        if (enabled) {
          let { dataCodeRowMap, dataCodeRowArr } = self.recursiveDataCodeRowMap(data, calcAndConstraintItemCodeField)
          // self.performGetDataAll({ data, dataCodeRowMap, dataCodeRowArr, getDataRenderMap, tableId: self.id, row }) // 渲染取数
          self.performColFormulaSingle({ data, dataCodeRowMap, dataCodeRowArr, colFormulaConfig, tableId: self.id, row, calcColType: 'lastLevel' }) // 行内计算
          let gradedSummaryRow = dataCodeRowMap[row['item__pNestCode'].split('_')[0]]
          if (gradedSummaryRow) {
            self.performGradedSummarySingle({ data, dataCodeRowMap, dataCodeRowArr, gradedSummaryFields, row, tableId: self.id }) // 分级汇总
          }
          self.performColFormulaSingle({ data, dataCodeRowMap, dataCodeRowArr, colFormulaConfig, row, tableId: self.id, calcColType: 'parent' }) // 父级行内计算
          self.performRowCodeFormula({ data, dataCodeRowMap, dataCodeRowArr, rowCodeFormulaConfig, tableId: self.id }) // 单元格交叉计算
          // // self.performRowConstraintFormula({ data, dataCodeRowMap,dataCodeRowArr, rowConstraintConfig, tableId: self.id }) // 行内约束
          // // self.performCellConstraintFormula({ data, dataCodeRowMap,dataCodeRowArr, cellConstraintConfig, tableId: self.id }) // 单元格约束
  ...
  performAfterEditClosedCalculate({ row, rowIndex, $rowIndex, data, column, columnIndex, $columnIndex }) { // 行改变事件执行单元格计算并刷新视图
    // 执行顺序： 取数-->行内计算-->分级汇总-->父级行内计算-->单元格交叉计算(涉及的行计算)-->行约束-->单元格约束
    let self = this
    const { property } = column
    return new Promise((resolve, reject) => {
      row = self.reductionColFormula(self.calculateConstraintConfigIn.colFormulaConfig, row, property)
      if (self.gradedSummaryFields.length) {
        let gradedSummaryRow = self.tableDataMap[row['item__pNestCode'].split('_')[0]]
        if (gradedSummaryRow) {
          self.performGradedSummarySingData(gradedSummaryRow, self.gradedSummaryFields)
        }
      }
  ...
  recursiveDataCodeRowMap(data, codeKey, levelNo = 1, pCode = '', obj = { dataCodeRowMap: {}, dataCodeRowArr: [] }) { // 递归生成数据code映射
    ...
    data.forEach((row, index) => {
      if (extendMapInfoField) {
        ...
        row['item__pNestCode'] = (pCode ? pCode + '_' + row[codeKey] : row[codeKey])
      }
      obj.dataCodeRowMap[row[codeKey]] = row
      obj.dataCodeRowArr.push(row)
      if (Array.isArray(row.children) && row.children.length) {
        row['last__lever'] = 0
        self.recursiveDataCodeRowMap(row.children, codeKey, levelNo + 1, row[codeKey], obj)
      } else {
        row['last__lever'] = 1
      }
    })
  ...
  performColFormulaAll({ data, dataCodeRowMap, dataCodeRowArr, colFormulaConfig, tableId, calcColType }) { // 执行 行计算
    ...
    dataCodeRowArr.forEach((row) => {
      self.performColFormulaSingle({ data, dataCodeRowMap, dataCodeRowArr, colFormulaConfig, tableId, calcColType, row })
    })
    return data
  },
  performColFormulaSingle({ data, dataCodeRowMap, dataCodeRowArr, colFormulaConfig, tableId, calcColType, row }) { // 执行 单条 行计算
    // calcColType  执行  lastLevel:最底级，parent:父级 all:全部
    if (this.isEmptyObj(colFormulaConfig)) {
      return data
    }
  ...
  performGradedSummarySingle({ row, gradedSummaryFields }) { // 执行 单条数据分级汇总计算
    if (!(Array.isArray(gradedSummaryFields) && gradedSummaryFields.length)) {
      return row
    }
    let self = this
  ...
  performRowConstraintFormula({ data, dataCodeRowMap, rowConstraintConfig, tableId }) { // 执行 行内约束
    this.genarateConstraintMap(rowConstraintConfig, data, dataCodeRowMap, tableId)
  },
  performCellConstraintFormula({ data, dataCodeRowMap, cellConstraintConfig, tableId }) { // 执行 单元格约束
    this.genarateConstraintMap(cellConstraintConfig, data, dataCodeRowMap, tableId)
  }
}
const constraintTool = {
  // 约束部分 行内约束
  // 底级取数-->行计算-->分级汇总-->父级行计算-->单元格交叉计算-->约束
  genarateConstraintMap(constraintConfig, tableData, dataCodeRowMap, tableId) {
    // 生成约束map
    let self = this
    ...
    this.each(tableData, (rowIndex, row) => {
      ...
      self.genarateRowConstraintMap(self.deepCopy(constraintConfig.row || {}), row, self.constrainRowDataMap[row[calcAndConstraintItemCodeField]], tableId)
    })
  },
  genarateRowConstraintMap(rowConstraintConfig, row, dataMap, tableId) {
    // 生成单条数据约束map
    let self = this
    this.each(rowConstraintConfig, (key, item) => {
      self.parsingRowConstraintConfigToMapData(rowConstraintConfig[key], row, dataMap, tableId)
    })
  },
  parsingRowConstraintConfigToMapData(rowConstraintConfig, row, dataMap, tableId) {
    // 生成单条数据约束map
    let self = this
  ...
  replaceRowEpressionValue(expression, row, tableId) {
    ...
    if (rowsKey.length) {
      for (let keyC in rowsKey) {
        ...
        expression = expression.replace(reg, row[keyF] === null || row[keyF] === undefined ? '' : row[keyF])
      }
    }
    return expression
  },
  ...
  // 约束部分 单元格间约束
  genarateCellConstraintMap(cellConstraintConfig, data, dataCodeRowMap, tableId) {
    let { constrainRowDataMap } = this
    self.each(cellConstraintConfig, (itemkey, item) => {
      let self = this
  ...
  getUpdatedDataAfterConstraint(tableData, constrainRowDataMap, dataCodeRowMap) { // 更新表格数据
    let self = this
    let calcAndConstraintItemCodeField = this.calculateConstraintConfigIn.calcAndConstraintItemCodeField
    self.each(tableData, (rowIndex, row) => {
      ...
      if (Array.isArray(row.children) && row.children) {
        self.getUpdatedDataAfterConstraint(row.children, constrainRowDataMap, dataCodeRowMap)
      }
    })
    return tableData
  }
...
const exportAndImportFn = {
  ...
  triggerExportOption(obj) {
    ...
    const selection = this.selection
    this.exportModalData = obj || Object.assign({
