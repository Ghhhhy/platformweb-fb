...
function getCellLabel(column, index, cellValue) {
  if (cellValue) {
...
function exportXLSX(params) {
  ...
  const rowList = datas.map((item,index) => {
    columns.forEach((column) => {
      item[column.id] = getCellLabel(column, index,item[column.id])
    })
    return item
  })
  if (isFooter) {
