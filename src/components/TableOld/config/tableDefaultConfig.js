...
export const pagerConfig = {
  align: 'right',
  // pageSizes: [10, 20, 50, 100, 500, 1000, 1000000],
  pageSizes: [
    {
      label: '10条',
      value: 10
  ...
  //     PrevJump, PrevPage, Number, JumpNumber, NextPage, NextJump, Sizes, Jump, FullJump, PageCount, Total
  layouts: [
    'Sizes',
    'PrevJump',
    'PrevPage',
...
export const footerConfig = {
  ...
  footerMethod({ columns, data, $table, $grid }) {
    ...
    let disabledMoneyConversion = $grid.$parent.footerConfigIn.disabledMoneyConversion
    ...
    return footerData.map((row) => {
      return row.map((item, index) => {
        if ((columns[index].own.combinedType + '').split(',').filter((item) => { return !!(item + '').replace(/null|undefined|true|false/ig, '') }).length) {
          let result
          if (disabledMoneyConversion) {
            result = Number((item).toFixed(2)).toLocaleString()
          } else {
