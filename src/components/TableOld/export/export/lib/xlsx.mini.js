...
*/
    ...
    var write_num = (function make_write_num() {
      ...
      function dec(val, d) {
        var _frac = val - Math.floor(val); var dd = Math.pow(10, d)
      ...
      while (i < fmt.length) {
        switch ((c = fmt.charAt(i))) {
          ...
          case 'A': case 'a': case '上':
            ...
            if (fmt.substr(i, 3).toUpperCase() === 'A/P') { if (dt != null) q.v = dt.H >= 12 ? 'P' : 'A'; q.t = 'T'; hr = 'h'; i += 3 } else if (fmt.substr(i, 5).toUpperCase() === 'AM/PM') { if (dt != null) q.v = dt.H >= 12 ? 'PM' : 'AM'; q.t = 'T'; i += 5; hr = 'h' } else if (fmt.substr(i, 5).toUpperCase() === '上午/下午') { if (dt != null) q.v = dt.H >= 12 ? '下午' : '上午'; q.t = 'T'; i += 5; hr = 'h' } else { q.t = 't'; ++i }
...
*/
  ...
  function zipentries(zip) {
    var k = zip.FullPaths || keys(zip.files); var o = []
  ...
  var unescapexml = (function() {
    ...
    var encregex = /&(?:quot|apos|gt|lt|amp|#x?([\da-fA-F]+));/ig; var coderegex = /_x([\da-fA-F]{4})_/ig
  ...
  function parse_rpr(rpr) {
    var font = {}; var m = rpr.match(tagregex); var i = 0
    var pass = false
    if (m) {
      for (;i != m.length; ++i) {
  ...
  var parse_rs = (function() {
    var tregex = matchtag('t'); var rpregex = matchtag('rPr')
    /* 18.4.4 r CT_RElt */
    function parse_r(r) {
  ...
  var rc_to_a1 = (function() {
    ...
    function rcfunc($$, $1, $2, $3) {
      ...
      var R = $2.length > 0 ? parseInt($2, 10) | 0 : 0; var C = $3.length > 0 ? parseInt($3, 10) | 0 : 0
...
*/
  ...
  function sheet_add_dom(ws, table, _opts) {
    ...
    var _R = 0; var R = 0; var _C = 0; var C = 0; var RS = 0; var CS = 0
    if (!ws['!cols']) ws['!cols'] = []
    for (; _R < rows.length && R < sheetRows; ++_R) {
      ...
      for (_C = C = 0; _C < elts.length; ++_C) {
        ...
        if (opts.dense) { if (!ws[R + or_R]) ws[R + or_R] = []; ws[R + or_R][C + or_C] = o } else ws[encode_cell({ c: C + or_C, r: R + or_R })] = o
  ...
  function make_json_row(sheet, r, R, cols, header, hdr, dense, o) {
    var rr = encode_row(R)
    var defval = o.defval; var raw = o.raw || !Object.prototype.hasOwnProperty.call(o, 'raw')
    var isempty = true
    var row = (header === 1) ? [] : {}
    if (header !== 1) {
