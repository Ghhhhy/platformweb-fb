        ...
        contentMethod({ items, row, rowIndex, $rowIndex, column, _columnIndex, $columnIndex, type, cell, $event }) {
          try {
            ...
            if (typeof (row) === 'object') {
              // 调整
              if (row && column.property && render && render.name === '$treeinput' && row[column.property]) {
              ...
              } else if (row && column.property && render && (render.name === '$vxeMoney' || render.name === '$vxeCalculate') && row[column.property]) {
