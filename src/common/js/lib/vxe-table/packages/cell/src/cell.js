...
export const Cell = {
  ...
  renderExpandData(h, params, fixedType) {
    const { $table, column } = params
    const { slots, contentRender } = column
    if (slots) {
      if (slots.content) {
        return slots.content.call($table, params, h, fixedType)
      }
      // 在 v3.0 中严格支持 content
      if (slots.default) {
        return slots.default.call($table, params, h, fixedType)
      }
    }
    if (contentRender) {
      const compConf = VXETable.renderer.get(contentRender.name)
      if (compConf && compConf.renderExpand) {
        return compConf.renderExpand.call($table, h, contentRender, params, { $grid: $table.$xegrid, $table }, fixedType)
      }
    }
    return []
  },
