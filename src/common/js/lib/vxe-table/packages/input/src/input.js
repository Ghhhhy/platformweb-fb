...
function isDateDisabled (_vm, item) {
  ...
  return disabledMethod && disabledMethod({ type: _vm.type, actionType: _vm.datePanelType, date: item.date })
}
...
export default {
  ...
  methods: {
    ...
    triggerEvent(evnt) {
      ...
      this.$emit(evnt.type, { $panel: $refs.panel, value: inputValue, $event: evnt }, evnt)
    },
    emitUpdate(value, evnt) {
      const { inputValue } = this
      if (XEUtils.toString(inputValue) !== value) {
        this.$emit('input', inputValue)
        this.$emit('change', { value: inputValue, $event: evnt })
      }
    },
    inputEvent (evnt) {
    ...
    numberKeydownEvent (evnt) {
      ...
      if (isUpArrow || isDwArrow) {
        ...
        evnt.target.value = this.transInputToNumber(evnt.target.value)
        // this.inputValue =
      }
    },
